...
## R86 — Frogger + Neo Jump + Agent Chase + Rhythm Hacker + 3 Globals (2026-04-21 → 2026-04-22)

Batches-of-2 cadence grew into batches-of-4 under Tom's faster test cadence. Phase opened with Matrix Frogger + Neo Jump (the originally-scoped 2 games), then Agent Chase polish was lifted from R87 mid-phase (Tom's 2026-04-22 verdict: mechanics good, 2 fixes needed — scoreboard + middle-box ghost release), then Rhythm Hacker was lifted late in the day when Tom's screenshot revealed a P0 pair (P-key QWOP / pause collision + hit-zone obscured by iPod dashbar chrome → game literally unplayable). 5 streams (G globals, F Frogger, N Neo Jump, A Agent Chase, R Rhythm Hacker, V verification) shipped clean in strict order with no regressions.

### Stream G — NEW Globals (3 commits)

- **G1 [P0] Neo Jump high-score persistence — defensive second write path** — `a1aec45`. Symptom matched R85.G1 exactly but only on Neo Jump; mirrored Metris's belt-and-suspenders pattern — direct `saveSystem.updateGameSave('neoJump', {...})` inside `playerDeath.onComplete` in addition to the existing `reportScore` event so highScore + stats land via two independent routes. 3 regression tests.
- **G2 [P1] click-to-focus required** — `9201848`. Root cause was subtler than "canvas isn't focused": R82 iPod dashbar's `onKeyDown` rove handler called `stopPropagation` on arrow/Enter/Space, which in React 17+ stops the native event too, so Phaser's window-level listener never fired until the user clicked into the canvas. Fix: widened `pendingSurfaceFocus` to include `'game'`, `handlePlayPress` captures `'game'` not `'dashbar'`, plus a post-play `useEffect` with `MutationObserver` focuses the Phaser container the moment it appears (handles React.lazy + Suspense late-mount). All three `PhaserGame.tsx` focus call-sites upgraded to `preventScroll: true`.
- **G3 [P2] WebGL warnings arcade-wide** — `55a5bcb`. Extracted `PHASER_RENDER_DEFAULTS` (`premultipliedAlpha: false` + `mipmapFilter: ''`) into `src/lib/phaser/types.ts` + spread into every game's render block (including the 5 configs that previously only set `pixelArt: true`). 12-game `it.each` enumeration regression test. Turned out the warnings fire on `pixelArt: true` games too — those two keys are WebGL-context-level, independent of sprite filtering.

### Stream F — Matrix Frogger polish (7 commits + 4 safety-nets)

- **F1 [P0] Finish-line crash** — `7c39cf1`. Three-way tween/physics/state-machine race: `movePlayer()` reset `playerRow=0` before its 150ms hop tween started, so `checkProgress()` fired a second reset tween fighting the hop tween; physics body kept interpolating through road lanes firing overlap → death. Fix: extracted `triggerLevelUp()` gated by `isLevelingUp` flag — kills in-flight tweens, disables body during reset, re-arms on onComplete; belt-and-suspenders early-return in `handleEnemyCollision` + `handleInput`. 8 regression tests.
- **F2 [P1] Countdown gate on spawn arming** — `83e24b2`. Mirror of R85.M2 for Frogger: gameplay arming funnelled through new `armGameplay()` as countdown onComplete + defence-in-depth early-return in `spawnEnemy` + `spawnPills` on `isCountingDown/isGameOver/isLevelingUp`. 9 tests.
- **F3 [P1] Kung Fu HUD re-siting** — `f1464aa`. 24×24 icons at `y=565` were only 11px clear of canvas floor, iPod bezel clipped. Moved whole HUD into top-left gutter alongside Score/Distance/Combo. Exploits Frogger's 25° perspective taper (left strip `x<120` stays dark through rows 0-2). 6 tests.
- **F4 [P1] Vehicle hit-box clamp** — `232a1db`. Quantified the encroachment: `setOrigin(0.5, 1)` + default body anchoring put body.top 7.5px past the lane into the row-above safe zone. New `HITBOX` config block (0.75 lateral ratio, 0.85 height ratio) + `applyEnemyBody(enemy, row)` helper re-centres body on `rowToY`. 6 tests.
- **F5 [P1] Game-objects legend in menu** — `b1d74d0`. Adopts Snake/Pong/Invaders pattern: 6-icon row (AGENT → SENTINEL → KUNG FU → POINTS → POWER-UP → NEO, danger-first order) at ratio 0.64. 18 tests in new `MenuScene.test.ts` locking ratios + clearance + textureKey membership.
- **F6 [P2] Multi-level polish — Tom-signed-off** — *"im happy with matrix frogger, its good"* (2026-04-22). 4 safety-net commits shipped ahead of Tom's tick (`b9ee461`/`f720982`/`81684a1`/`f77a8c3`) add +32 regression tests covering state persistence, timer isolation, difficulty-ramp arithmetic, playerDeath juice contract.
- **F7 [P1] Coverage refresh** — `92647b0`. +11 tripwires: finish-line guards (checkProgress/handleInput/triggerLevelUp/showLevelUpText), update() countdown gate, hit-box visible-bounds extensions (X-offset symmetry, 64×64 agents under tight lanes, defensive null guards).

### Stream N — Neo Jump polish (5 commits + 3 safety-nets)

- **N1 [P1] Difficulty rebalance** — `9dfadeb`. 5 axes: tutorial zone 500→800m, spawn density ~40% cut, enemies slowed (50-100→40-75), jetpack buffed (120 fuel / 8 regen / 25 drain = +44% airtime), spawn fairness (`MIN_HORIZONTAL_SPACING_FROM_PLAYER=80` with 5-retry bail-out). 19 tests.
- **N2 [P1] Fall-death threshold 50m** — `b95f7ad`. New `PLAYER.MAX_FALL_DISTANCE_METRES=50` + `fallApexY` tracked since last platform contact; reset in `handlePlatformCollision` to contact y (not platform y — anchors next bounce's budget where Neo stood). Death check fires before off-screen so player sees death anim on-camera. 13 tests.
- **N3 [P1] In-game controls legend** — `91d217d`. Both MenuScene panel (5 rows text-only two-column) AND fading in-play overlay (300ms fade / 3s hold / 800ms fade, ≤5s total inside 5s countdown). 25 tests.
- **N4 [P1] Coverage refresh** — `482a359`. +12: update() loop early-return gates, playerDeath juice literals (shake 200/0.012 + flash 120,255,0,0,0.25), handleInput wrap arithmetic, canCollideWithPlatform one-way collider (Doodle Jump physics pillar), isDying guards on collision paths.
- **N5 [P1] Flow second-pass** — `bb771ee`. Tom's 2026-04-22 re-playtest: *"still too difficult early, cannot get momentum, 5s restart too heavy"*. 3 levers: tutorial strip extended (SPAWN_ALTITUDE 800→1000, SPAWN_CHANCE_BASE 0.018→0.013), retry-within-30s shortcut (5s→2s countdown via `registry.neoJumpLastDeathAt` timestamp), opening-beat spawn protection (1000ms + flash tween + `isSpawnProtected()` guards on maybeSpawnEnemy AND handleEnemyCollision). 27 tests.

### Stream A — Agent Chase polish (3 commits + 2 safety-nets, lifted from R87)

- **A1 [P1] Scoreboard live-refresh — root-caused to useSaveSystem** — `9e003b8`. Diagnosis was subtler than the plan's Zustand guess: NO Zustand store for save data. Every `useSaveSystem()` caller got its own `useState` bucket; writes on PhaserGame's instance landed in localStorage but never propagated in-memory to App's instance, so Scoreboard modal showed stale snapshot. Fix: converted `useSaveSystem` to module-level singleton — `sharedSaveData` + `useSyncExternalStore(subscribeSaveData, getSaveDataSnapshot)` + `setSharedSaveData` funnel. 9 in-hook `setSaveData(...)` sites migrated. Exported `__resetSaveSystemForTest` for vitest isolation. Defensive Agent Chase `updateGameSave` shipped as belt-and-suspenders. +12 tests (7 cross-instance sync + 5 defensive-write).
- **A2 [P1] Middle-box ghost-release + pathfinding override** — `cff647d`. Bug was two-fold: (1) release was 5s-interval (later agents never released within a level's playtime on rapid retries); (2) greedy Euclidean scatter-target heuristic couldn't exit the house — johnson at `(14, 13)` with scatterTarget `(0, 30)` picked LEFT (18.38) over UP through gate (18.44) on a 0.06-unit tiebreak, looping into a dead-end pocket. Fix: `RELEASE_DOT_THRESHOLDS=[0,10,30,60]` staggered release + `EXIT_TILE={x:14,y:11}` one-tile-above-gate override prepended to `getAgentTarget` for scatter+chase states inside house (frightened+returning exempt). 26 tests.
- **A3 [P1] Coverage refresh** — `b30b149`. +12: A1 null-chain resilience (saveData=undefined first-run paths, highScore-promotion-before-updateGameSave ordering, Math.floor(duration/1000) locked), A2 structural pairing invariants (thresholds.length=AGENT_TYPES count, agent name order), static anti-regression (playerDeath has exactly one updateGameSave call, releaseAgents uses while-loop not if, getAgentTarget calls isInsideGhostHouse). 2 A3+ safety-nets (`29ad5cd`, `b7abe7f`) add +20 tripwires locking releaseAgents signature purity + useSaveSystem singleton structural invariants.

### Stream R — Rhythm Hacker unblock (4 commits + 1 safety-net, lifted late from R87)

- **R1 [P0] P-key QWOP / pause collision** — `1b35bd5` (batched with R2). Arcade-wide `P=pause` binding intercepted every lane-4 press, pausing the song mid-note. New `protected getPauseKeyCode(): number | null` virtual on `BaseScene` (defaults to `KeyCodes.P`) consulted inside `_bindCommonKeys`; RhythmHackerGameScene overrides to return null. Pause stays reachable via iPod dashbar `PAUSE_REQUEST_EVENT`. MenuScene footer stripped of `P: Pause` hint. 6 tests + 6-test safety-net (`8b0f849`) with arcade-wide uniqueness scan (exactly 1 override across 11 other Phaser scenes) + literal-return body shape lock.
- **R2 [P0] Hit-zone dashbar clearance** — (batched with R1). Tom's screenshot showed pause button `II` sitting directly on lanes 2/3 hit-zones. Chose in-canvas reposition over shrink-HEIGHT (kept HEIGHT=700 to avoid a cascade through BootScene / MenuScene / GameOverScene). Paired dial moves: `HIT_LINE_Y: 640→560` + `SPAWN_HEIGHT: -50→-130` so `(HIT_LINE_Y - SPAWN_HEIGHT)/SPEED*1000` stays exactly 1725ms (approach feel preserved). 7 tests lock the paired-dial travel-time invariant.
- **R3 [P1] MenuScene RESONANCE layout** — `c1b0010`. Classic "anchored-to-bottom via HEIGHT - offset" bug: HOW-TO-PLAY band assumed 4 tracks, RESONANCE was 5th → title at y=520 spanned over the card at y=510-570. Refactored 9 layout Y values into named exported constants (TRACK_START_Y=150, TRACK_SPACING_Y=90, HOW_TO_PLAY_TITLE_Y=560 with 13px clearance from last track). 15 tests including Tom-facing `lastTrackBottomEdge + CLEARANCE_PX ≤ titleTop` tripwire — adding a 6th track without moving the title breaks the gate immediately.
- **R4 [P1] Coverage refresh + processHit game-over bug fix** — `55e7855`. Audit surfaced a legitimate bug at `GameScene.ts:684` in `processHit→miss→health-depleted`: `reportScore(score, score)` was lying about highScore + `gameOver(…, undefined, …)` mismatched L530 + L1147. A1's singleton masked it but a regression would re-expose. Fix: promote highScore + pass real value to both calls. +17 tests including direct regex tripwire on the pre-R4 bug string.

### Stream V — Verification + Baselines (1 commit)

- **V1** — `141ac31`. All 7 gates green on darwin: tsc clean, lint 0-err / 4 pre-existing warnings, build 7.54s, unit 3089/3091 pass across 59 files in 16.44s, chromium E2E 74/3-skipped in 4.1 min, clean re-run against fresh baselines 74/3 stable. 78 darwin baselines regenerated across 12 playthrough specs + 4 visual/ spec dirs. **Critical finding**: plain `--update-snapshots` no-ops in Playwright 1.58+ (flag defaults to `changed`), project tolerance (maxDiffPixels 200000 / threshold 0.5) absorbs ~20% pixel divergence so intentional R86 shifts (HIT_LINE_Y 640→560, Neo Jump controls overlay, Frogger legend, MenuScene RESONANCE lift) all passed silently against April-13 / R78.5 baselines — required explicit `--update-snapshots=all` to force on-disk refresh.

### Test count trajectory

Phase entry: **2,754+ unit tests** / 54 files. Phase exit: **3,115+ pass** / 60 files. Net **+361 regression tripwires** across 31 commits (17 core + 14 safety-nets).

### Still-open at R86 exit

- R86 terminator phrase — Tom-side; writes `R86 COMPLETE — Frogger + Neo Jump polish + globals shipped` to Status after hand-playtest confirms the 4 games feel right.
- Agent Chase level-advance bug surfaced 2026-04-23 post-V1 (see Tom's in-session screenshot — full maze, 0 dots remaining, game stuck on Level 1 with 2570 score). Not in R86 scope since ticket-less at phase entry — promoted to **R87.AC1** Stream AC.

**Total R86 footprint**: 31 commits across 5 streams + ~361 new unit tests + extensive discovered-work log covering 8-game defensive-write cross-audit, 7 safety-net cadence cases (F6/N5/N2+/F6+/F6++/F6+++/N4+/A3+/A1+/R1+).
