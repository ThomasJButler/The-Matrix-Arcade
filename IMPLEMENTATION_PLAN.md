...
## Status: R82 open — R82.13 continuous-improvement: perf micro-optimisations. Filled three composite-layer/containment gaps without disturbing visuals: (1) `.clickwheel-zone` gains `will-change: transform` to match `.dashbar-btn` (4 small wheel zones promoted to GPU layers — minimal layer cost for noticeably smoother press scale), (2) `.ipod-clickwheel-region` gains `contain: layout style` so the wheel↔dashbar AnimatePresence morph can't trigger reflow in the surrounding `.ipod-body`, (3) `.ipod-loading-screen` gains `contain: layout style paint` (Suspense fallback with continuous scanline + 7 char-pulse animations is fully self-contained, paint-isolating it lets the compositor skip ancestor invalidation while the game chunk loads). Updated the `prefers-reduced-motion` reset to add `.clickwheel-zone` to the `will-change: auto` release list — proper memory hygiene when animations are disabled. Gates green (tsc clean, lint 0 errors, App.test.tsx 19/19, build 7.37s clean). Ralph does NOT write terminator — Tom's manual call post-review.
