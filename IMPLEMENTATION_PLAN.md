...
## Status: **R85 open** (2026-04-20 — Tom's playtest notes on Matrix Invaders + Metris filed). R85 = **Invaders + Metris polish + 2 NEW globals** carved out of the originally-planned 8-game polish phase because Tom wants to test the remaining 6 games first before committing to their scope. Loop cap: **20 iterations**. **Stream M cracked open 2026-04-21** — M1 + M2 shipped as a single countdown-gated-timer fix (both traced to `create()` arming `startDropTimer` before `startCountdown`, so the 400ms level-1 drop timer raced the 5s countdown and the first piece slid ~12 rows before the player could react; countdown text at depth 200 was firing correctly but Tom perceived it as missing because gameplay had visibly already begun). Fix moves `startDropTimer()` into the countdown's `onComplete` callback so no ticks fire during the 5s window, plus defence-in-depth `if (this.isCountingDown || this.isPaused) return;` early-exit in `dropTick()` itself against any future refactor that re-arms a pre-countdown timer. 9 new tests in `§ R85.M1 / M2 Countdown-gated drop timer` lock the contract: dropTick no-ops during countdown, no-ops during pause, resumes post-countdown, respects isGameOver gate independently, create() calls startDropTimer only via startCountdown's onComplete (ordering assertion), startDropTimer NOT called when countdown callback never fires (regression tripwire for the original bug), 5-second arity on startCountdown matches arcade convention, all 7 tetromino types spawn at y=0, 12-unguarded-ticks pathological case confirmed piece stays at top. **Stream G globals — both shipped 2026-04-21** (G1 high-score persistence + G2 dashbar trophy). **R85.I1 (enemy sprite redesign) shipped 2026-04-21**. **R85.I2 bullet-time HUD shipped 2026-04-21**. **R85.I3 bigger enemy bullets shipped 2026-04-21**. **R85.I4 power-up shooter visibility invariant shipped 2026-04-21** — three contributing patterns identified and fixed: (a) `activatePowerUp('shield')` mutated `player.setTexture('player_shield')`/`setTint(MAGENTA)`, (b) `updateHUD()` re-asserted that mutation every frame (fighting with the damage-blink tween), (c) `spawnPowerUp()` created `repeat:-1` yoyo tweens with no cleanup on pickup — dangling tween targeting a destroyed sprite silently corrupted the shared tween manager, which over minutes of gameplay could leave the player sprite stuck invisible/tinted. Fix architecture: separation of concerns — player sprite is canonical and **never mutated** by power-ups; shield visual is its own `shieldAura` Graphics layer (depth 2, below player at depth 3, magenta fill α=0.12 + stroked ring α=0.9, pulsing 0.75→1.0 α over time) rendered as a halo around the ship. New `restorePlayerVisuals()` invariant centralises `setVisible(true) + setAlpha(1) + clearTint()` and is called at every state-window boundary (post-shield-break, post-blink-cleanup, end of every `activatePowerUp()` switch). Tween cleanup added at all three power-up destroy sites (`checkPowerUpCollisions`, `updateFieldPowerUps`, `shutdown`) via `this.tweens.killTweensOf(pu.sprite)` before destroy — kills the dangling `repeat:-1` yoyo before the sprite is gone. Removed the entire `spriteMode` tint / non-spriteMode setTexture block from `updateHUD` — that whole branch was re-writing player visuals every frame on the hope that the shield effect persisted correctly. Regression tripwire: 14 new tests in `GameScene.test.ts § R85.I4 Power-up Shooter Visibility Invariant` — outcome-based `assertPlayerVisibleAndOpaque()` helper checks visible=true + all setAlpha args=1 + all setVisible args=true + no setTexture call + no setTint call; tests for each power-up type (shield/rapidFire/scoreMultiplier/bomb) individually + the canonical Tom-repro "stacking every power-up type leaves the player visible and opaque" + 10-consecutive-shields stress test + shield aura show/hide/pin-to-player + shield-break + updateHUD non-mutation + pickup/offscreen tween-kill tripwires. All 2585 unit tests green (2 pre-existing todo — now 12 new net I4 tests on top of the base), tsc clean, lint clean (0 err, 4 pre-existing warnings unchanged), build green in 7.57s. **R85.I5 menu start-button / control-hint overlap shipped 2026-04-21** — subclass had stacked three hint lines at `height * 0.72 + i*22` (y = 324/346/368 on the 450-px canvas) which painted squarely on top of the shared START button centred at `BaseScene.MENU_START_BUTTON_Y_RATIO = 0.75` (~50 px tall → y band 312-362). Root-cause pattern: Invaders was the lone outlier; every other subclass (Snake, MatrixCloud, NeoJump, AgentChase, Frogger, CloudJumper, Pong) places hint lines in the conventional 0.52-0.64 instruction band that BaseScene's comment explicitly documents as the button's clear-air zone. Fix matches that convention — new exported `CONTROL_HINT_Y_RATIOS = [0.52, 0.58, 0.64]` tuple on the subclass, hints painted at those ratios, MATRIX green hex colour arg made explicit to match Snake/NeoJump signature. Regression tripwire: new `src/components/games/phaser/MatrixInvaders/scenes/MenuScene.test.ts` — 11 tests covering (a) static tuple invariants (length, in-band 0.52-0.64, strictly increasing, does-not-regress-to-0.72 tripwire), (b) button-clearance maths on both 450-px and 400-px canvases, (c) `create()` wiring — createMatrixText called 3× with correct y per ratio, correct hint strings in order, no hint y lands inside button band, ratios scale with canvas height rather than hard-coded pixels. Phaser prototype chain is broken by the mock Scene factory so tests use the `collectPrototypeMethods` rebind trick (same as VortexPongMenuScene.test.ts) plus a temporary `BaseMenuScene.prototype.create = vi.fn()` stub so only the subclass body runs. All 2596/2598 unit tests green (2 pre-existing todo — +11 net I5 tests), 54 test files, tsc clean, lint clean (0 err, 4 pre-existing warnings unchanged), build green in 13.16s. **R85.I6 power-up legend shipped 2026-04-21** — root cause was teaching-surface absence, not visual bug; players pick up a coloured orb, ACTIVE banner names the verb but does not explain effect or duration. Fix adopts Pong R84.P5's 3-method stack (show/hide/clear + `prefersReducedMotion`) because its "on every pickup" trigger matches Tom's *"each collect + activate"* phrasing better than Snake R83.S1's "once at startup" model. New `POWERUP_LEGEND` config in `MatrixInvaders/config.ts` — `ENTRIES` tuple with 4 rows (name · effect · duration), 4s display, 200/400ms fade, `BASE_Y_RATIO=0.70` (y=315-363, clear air between central banners at 0.30/0.40/0.45 and player at y=410). Active row paints at α=1 in `POWERUP_DEFS[type].color`, other 3 dim to α=0.55. Cancellation-token guard on fade-out onComplete (`if (this.powerUpLegend === targets)`) stops stale fades from wiping fresh legends when pickups queue within FADE_OUT_MS. `prefers-reduced-motion` skips fade tweens (alpha set directly). Wired into `checkPowerUpCollisions` after `activatePowerUp` plus `clearPowerUpLegend` into shutdown + resetState. Regression tripwire: 24 new tests in `GameScene.test.ts § R85.I6 Power-up Legend HUD` covering rendering contract (4 Text objects, correct depth/font/y-spacing), active-row highlighting, back-to-back refresh + cancellation-token guard, cleanup paths (shutdown/resetState/delayedCall auto-hide), reduced-motion (alpha set directly), pickup wiring. Bug found during test shakedown: `prefersReducedMotion` needed a second `?.` after the matchMedia call — `window.matchMedia?.('(reduce)')?.matches` — first `?.` protects the call site, second `?.` protects the dereference when matchMedia returns undefined. Discovered-work note logged: Tom mentioned *"6 power-ups"* but Invaders ships only 4 — legend covers all 4, scope extension requires lockstep `POWERUP_LEGEND.ENTRIES` + `POWERUP_DEFS` updates. All 2620/2622 unit tests green, tsc clean, lint clean (0 err, 4 pre-existing warnings unchanged), build green in 7.56s. **R85.I7 atmosphere amp-up shipped 2026-04-21** (rain density 10→30, CRT scanline overlay, per-kill Matrix-green flash, every-5th-kill combo camera pulse — 24 tripwire tests). **R85.I8 boss verify + polish shipped 2026-04-21** — static audit surfaced 4 boss design bugs (fire cadence 6-7× lower than regular wave, no encounter scaling despite linear health/value ramp, no enrage mechanic despite red healthbar, hit/defeat juice mute). Fixes: `BOSS_FIRE_CHANCE` bumped 0.002→0.006 (3×), new `BOSS_FIRE_CHANCE_PER_ENCOUNTER=0.3` linear ramp, `BOSS_ENRAGE_THRESHOLD=0.25` + 2× fire-rate multiplier below that HP ratio (RED healthbar now carries real mechanical consequence), `hitBoss` 50ms/0.003 camera shake per hit, `defeatBoss` 350ms/0.012 shake + 300ms/0.3α green flash + particle counts scaling per encounter (`40 + (enc-1)×8` main, side 0.5× main, bottom 0.375×). `encounter: number` added to `BossState` so scaling reads from boss-local state. 17 new tests in `§ R85.I8 Boss Fire Cadence + Juice Scaling` with config invariants, deterministic Math.random-mocked cadence tests, enrage boundary tests (catch `<=` regression), hit/defeat juice args, particle proportional scaling, spawnBossWave encounter wiring. All 2667/2669 unit tests green, tsc clean, lint clean, build green in 14.48s. **R85.M3 bullet-time manual-only activation shipped 2026-04-21** — deleted the 3-line auto-trigger block in `handleScoring` that fired `activateBulletTime()` the moment the meter tipped to MAX; the existing `tryManualBulletTime()` + B-key wiring at `handleInput` line 776 was already correct. HUD readiness cue upgraded: `meterLabel` now flips to YELLOW_HEX when full-but-idle (was always PRIMARY_HEX), so label + meter fill + `'READY!'` timer text all pulse yellow in sync — unmissable B-prompt. **R85.M4 column spacing shipped 2026-04-21** — audit found 5 concrete issues behind Tom's vague note: right `panel_tall` backdrop was 80×240 (3× the 80×80 HOLD panel = top-heavy right wing), 50 px of dead air between HOLD panel bottom and the first stat label, BULLET TIME orphaned with 35 px gap from COMBO vs 25 px elsewhere, control hints hand-coloured `'#005500'` darker than `MATRIX_COLORS.DARK_GREEN_HEX`, zero visual grouping for loose stat/control text. Fix centralises all layout into a 15-constant block in `Metris/config.ts` (`PREVIEW_PANEL_W/H/Y_OFFSET`, `STATS_Y_START/ROW_H/LABEL_VALUE_GAP`, `BULLET_TIME_LABEL_Y/BAR_Y/BAR_W/BAR_H/TIMER_Y`, `HIGH_SCORE_LABEL_Y/VALUE_Y`, `CONTROLS_Y_START/ROW_H`, `STATS_PANEL_W/H`, `CONTROLS_PANEL_W/H`) then shrinks NEXT panel to 80×80 for symmetry, pulls stats block up 40 px (y=160→120) removing dead air, uniform 20 px STATS_ROW_H rhythm across SCORE/LEVEL/LINES/COMBO plus BULLET TIME, `MATRIX_COLORS.DREAD_GREEN_HEX` for 7 control hints (arcade-wide R83.CTRLS.17 ambient-text convention), and two subtle α=0.18 secondary `panel_tall` backdrops behind the left stats+bullet-time stack and the right HIGH SCORE+controls list to frame each HUD block without competing with primary focal points. `createHUD` now iterates `statLabels`/`ctrlLines` arrays instead of hand-placing every text node; `drawBulletTimeMeter` uses `barX = HOLD_X - BULLET_TIME_BAR_W / 2` so the bar is centred by construction rather than via a magic `-60` offset. Regression tripwire: 24 new tests in `§ R85.M4 Column Spacing Layout` across 6 sub-describes — preview-panel symmetry (W===H, `PREVIEW_PANEL_H < 240` direct tripwire for pre-R85.M4 bug, HOLD_X/NEXT_X symmetric around grid centre), left-column stats rhythm (STATS_Y_START within 20 px of HOLD panel bottom, STATS_ROW_H ∈ [12, 24], label-value gap < row gap), bullet-time block rhythm (gap from last stat in `[STATS_ROW_H/2, 2×]`, bar centred under HOLD_X via strokeRect.mock.calls introspection, bar W/H from config), right-column hierarchy (HIGH_SCORE_LABEL_Y === STATS_Y_START matched rhythm, CONTROLS_ROW_H < STATS_ROW_H, 7 ctrl rows fit in panel), createHUD wiring (add.text.mock.calls introspection catches hardcoded y-positions drifting from constants — 4 stat labels in order at correct y, 7 control colours refuse `#005500` via regex `/^#00[79a-f][0-9a-f]00$|^#00ff00$/`, uniform CONTROLS_ROW_H rhythm, BULLET TIME + HIGH SCORE at exact config-named y's), createGraphics panel symmetry (HOLD and NEXT preview both render `PREVIEW_PANEL_W×PREVIEW_PANEL_H`, NO image anywhere renders at h=240 direct tripwire, secondary panels α ≤ 0.25). All 150/150 Metris tests green (+24 net), 2708/2710 full suite green (2 pre-existing todo), tsc clean, lint clean (0 err, 4 pre-existing warnings unchanged), build green in 7.35s. Visual baseline regen deferred to R85.V1. **R85.I9 Invaders coverage refresh shipped 2026-04-21** — +23 cross-cutting integration tests across 6 sub-describes: drawHealthBar colour thresholds (green/yellow/red pulse), drawBulletTimeMeter render contract (fill width + ready pulse + active/refilling solid α), updateShieldAura multi-frame pinning + pulse formula, bomb × boss (20 HP, defeat path, non-lethal leave-alive), bomb × scoreMultiplier design invariant (panic button pays raw value), danger warning one-shot, wave-complete clears bullets + stacks WAVE_5/10/20, checkGameOverConditions no-op during wave transition, BULLET_TIME_SCALE propagates through updateEnemies. Fills the integration niche that per-task I1-I8 tripwires left open. All 2731/2733 unit tests green (2 pre-existing todo), tsc clean, lint clean, build 7.50s. **R85.M5 Metris coverage refresh shipped 2026-04-21** — +23 cross-cutting integration tests across 7 sub-describes (drop-speed modifier compounding, bullet-time lifecycle side-effects, line-clear → level-up → drop-timer cascade + meter saturation + tetris camera args, hold × bag integration, game-over save-system persistence round-trip with stats-merge max semantics, update() gate asymmetry pause/gameOver expose state vs countdown skips, getTestState contract). Fills the integration niche that per-task M1-M4 tripwires left open — mirrors R85.I9's role for Invaders. 173/173 Metris tests green (+23 net, +65 total across M1-M5), 2754/2756 full suite green, tsc clean, lint clean, build 7.71s. **R85.V1 baselines + full gate battery shipped 2026-04-21** — all seven gates green on darwin (tsc clean, lint 0-err / 4 pre-existing warnings, build 7.64s with 292 PWA entries, unit 2754/2756 pass across 54 files in 14.65s, chromium E2E 74/77 pass in 4.3 min with 3 known `.skip()` decorators, Invaders + Metris darwin baselines refreshed to depict R85 polish). **All R85 tasks (G1/G2 + I1-I9 + M1-M5 + V1) are now `[x]` — phase awaiting Tom's hand-playtest sign-off + terminator phrase** (Ralph cannot self-write). **R85 Discovered Work list expanded 2026-04-21** with the game-load snap transition polish item surfaced by `MANUAL_TESTING_CHECKLIST_Metris.md:18` (R83.G5 works but can be improved — CI-bucket-style polish candidate for post-R85 triage, not R85 scope). **R84 still awaiting sign-off** (R84.S7 `[DEFER]`, R84.CI bucket Tom-tick-only). R83.CTRLS umbrella also still pending Tom's CTRL-S end-to-end tick.
...
- [x] **R85.I7 [P2]** **Matrix-style atmosphere amp-up** — **SHIPPED 2026-04-21**. Tom's playtest: *"Needs more Matrix-style jazz / atmosphere"*. Root cause: the Invaders backdrop was a flat black fill plus a sparse 10-char matrix rain; the play field read as a blank canvas rather than a terminal inside the Matrix. Fix is a 4-lever procedural amp-up centralised in a new `ATMOSPHERE` config block so designer-level tweaks live in one place. (a) **Rain density 10 → 30**: fills the dead-black zones between enemy rows with drifting glyphs but stays below the readability ceiling (≤ 50 on 800×450). (b) **CRT scanline overlay**: one-shot `createScanlineOverlay()` paints ~150 horizontal 1-px fillRects at spacing 3, alpha 0.06, depth 8 — sits above gameplay (3-6) for the authentic phosphor-on-top look but below HUD banners (100) so score text stays crisp. Zero per-frame cost; skipped in `window.__TEST__` mode mirroring the BaseScene matrix-rain skip so E2E visual baselines stay stable. (c) **Matrix-green kill flash**: new `spawnKillFlash(x, y)` pushes a single 28×28 stationary rect into the existing `particles[]` pool with `vx=vy=0, life=0.3` so the existing `updateParticles` decay loop handles fade + teardown — no new lifecycle code. Depth 7 keeps the flash above explosion debris (6) so it reads as "killing energy" not another shard; flash uses `MATRIX_COLORS.PRIMARY` (green) rather than per-enemy tint so every kill brands with the series palette. (d) **Combo milestone camera pulse**: every 5th combo kill fires `cameras.main.flash(80ms, 0, 255, 0, …, α=0.08)` — too frequent pulses trigger nausea; milestone-only means every 5th/10th/15th kill gets subtle reward shaping. Pulse alpha (0.08) intentionally sits below wave-clear (0.15) and game-over (0.25) flashes so those remain the strongest punctuation moments. Scanline Graphics cleanup added to `shutdown()` (destroy + undefined reset) mirroring the R85.I4 shieldAura pattern — explicit teardown survives any future refactor that loses Phaser's implicit scene-GC. Regression tripwire: 24 new tests in `GameScene.test.ts § R85.I7 Atmosphere Amp-up` covering (i) config invariants (rain density floor 20, ceiling 50; scanline alpha `(0.03, 0.12]`; scanline depth in `(3, 100)`; kill-flash size > scaled enemy width; combo pulse every > 1; pulse alpha < wave-clear 0.15 — each invariant ties a design intent to a regression floor/ceiling), (ii) scanline overlay (fillRect call count = ⌈HEIGHT/SPACING⌉, depth + green + α matches config, `__TEST__` skip preserves E2E baselines), (iii) kill flash (vx=vy=0, life, size, colour, alpha, depth all wired to config; decays cleanly via `updateParticles` loop), (iv) combo pulse (no-pulse at combo=1, exact pulse at 5/10, no-pulse at off-milestone 6, Matrix-green RGB with configured duration + alpha), (v) `killEnemy` integration (produces PARTICLE_COUNT+1 particles = 20 explosion + 1 stationary flash), (vi) shutdown cleanup (destroys overlay, tolerates undefined test-mode path). Full suite: 2650/2652 unit tests green (2 pre-existing todo, +30 net from R85.I7), 54 test files, tsc clean, lint clean (0 err, 4 pre-existing warnings unchanged), build green in 7.65s. Visual baseline regen deferred to R85.V1 — the scanline overlay + denser rain WILL invalidate the existing Invaders baseline, which is the expected cost of the amp-up.
- [x] **R85.I8 [P2]** **Boss verify + polish** — **SHIPPED 2026-04-21**. Tom's testing-doc note was *"Boss (if any) fires correctly — not got to boss yet"*, leaving the mechanic unverified. Static audit surfaced four design bugs behind that deferred live-test: (1) Boss fire cadence was backwards — `BOSS_FIRE_CHANCE(0.002) × 60 × 3 barrels ≈ 0.36 shots/sec` was 6-7× lower than a regular 40-enemy wave at `ENEMY_FIRE_CHANCE(0.001) × 60 × 40 ≈ 2.4 shots/sec`, so the climactic boss felt less threatening than the wave it replaces; (2) Difficulty curve was incomplete — health scaled linearly (`50 + (encounter-1) × 10`) and value scaled linearly (`500 × encounter`) but fire rate, barrel count and movement speed were fixed, so a wave-25 boss with 140 HP fired at the same cadence as a wave-5 boss with 50 HP; (3) No enrage mechanic despite the visual language already being in place — `drawBossHealthBar` paints the bar RED at < 25% HP but no mechanic triggered at that threshold; (4) Hit and defeat juice was mute — `hitBoss` did a 100ms white tint but no camera shake (lost against the 120×60 sprite), `defeatBoss` spawned explosions and played sounds but no camera shake, no flash — even a trivial wave-complete transition (`shake(150, 0.006) + flash(150, 0,255,0, …, 0.15)`) had more camera feedback than killing the boss. No null-guard bugs found — `updateBoss`, `hitBoss`, `defeatBoss`, `checkPlayerBulletCollisions`, bomb path and shutdown all guard correctly. Fix architecture: (a) `BOSS_FIRE_CHANCE` bumped from 0.002 → 0.006 (3×) giving ~1.08 shots/sec at encounter 1 — half of a regular wave, credible as a climax without being oppressive; (b) new `BOSS_FIRE_CHANCE_PER_ENCOUNTER = 0.3` linear ramp so encounter 3 fires 1.6× faster than encounter 1, encounter 5 fires 2.2× faster — ramps the climax per 5-wave block without exponential unfairness; (c) new `BOSS_ENRAGE_THRESHOLD = 0.25` + `BOSS_ENRAGE_FIRE_MULTIPLIER = 2.0` — the RED healthbar now carries real mechanical consequence, classic enrage phase telegraphed via colour before it activates; (d) `hitBoss` camera shake via new `BOSS_HIT_SHAKE_DURATION/INTENSITY = 50ms/0.003` — intentionally quietest-that-registers because rapid fire would chain stronger shakes into nausea; (e) `defeatBoss` shake + green flash via new `BOSS_DEFEAT_SHAKE_DURATION/INTENSITY = 350ms/0.012` and `FLASH_DURATION/ALPHA = 300ms/0.3` — parity-plus with wave-complete, below game-over (500ms, 0.25α) so the camera punctuation hierarchy (hit < wave-complete < boss-defeat < game-over) reads correctly; (f) defeat particle counts scale per encounter via `BOSS_DEFEAT_MAIN_PARTICLES_BASE = 40 + (encounter-1) × 8`, side bursts 0.5× the main count and bottom burst 0.375×, so wave-25 bosses get materially bigger death throes than wave-5. New `encounter: number` field added to `BossState` interface so `updateBoss`/`defeatBoss` can read the scaling factor from boss-local state rather than re-reading `this.wave` (which will tick past the encounter value post-defeat — decoupling prevents a subtle off-by-one if the boss somehow outlives the wave tick). `spawnBossWave` stamps `encounter` when instantiating the boss inside its `delayedCall` callback. Regression tripwire: 17 new tests in `GameScene.test.ts § R85.I8 Boss Fire Cadence + Juice Scaling` covering (i) 6 config invariants (fire-chance floor ≥ 0.005 tripwire catches a regression back to 0.002, per-encounter mult > 0, enrage threshold inside (0, 0.5) matching the healthbar RED zone, enrage mult > 1, defeat shake > hit shake hierarchy, per-encounter particle bonus > 0), (ii) 4 fire-cadence tests using `vi.spyOn(Math, 'random').mockReturnValue(…)` with calibrated rolls between encounter-1 and encounter-3 rates (0.45 between 0.36 and 0.576) to deterministically assert encounter scaling, enrage doubling below 25% HP, and the boundary case that `healthPct === 0.25` does NOT enrage (prevents a stray `<=` regression), (iii) 2 hit-juice tests (shake fires with the configured args, shake fires even when the hit does not defeat — regression tripwire against gating shake on `health <= 0`), (iv) 4 defeat-juice tests (defeat shake with configured args, green flash with (0,255,0) RGB, main particle count equals `BASE + (encounter-1) × PER_ENCOUNTER`, side + bottom bursts scale proportionally to main — catches a regression where only the main burst scales), (v) 1 integration test asserting `spawnBossWave` populates `encounter` from `wave / 5` correctly by firing the stored `delayedCall` callback. Existing `createBoss` test helper updated to take `encounter` + `health` params with sensible defaults so pre-existing boss tests stay 1-line unchanged. All 2667/2669 unit tests green (2 pre-existing todo, +17 net from R85.I8), 54 test files, tsc clean, lint clean (0 err, 4 pre-existing warnings unchanged), build green in 14.48s. Live boss verification still blocked on Tom's playtest — a skilled player needs 3-5 min to reach wave 5, and the unit tests lock the mechanical contract but cannot assert subjective "feels threatening now".
- [x] **R85.I9 [P1]** **Unit-test coverage refresh** — **SHIPPED 2026-04-21**. I1-I8 each shipped with dense per-task tripwires (+112 tests total across Invaders); R85.I9 fills the **cross-cutting integration** niche — interactions between features where a seemingly-unrelated refactor could silently break a seam. Audit surfaced 10 concrete gaps: (a) `drawHealthBar` colour thresholds (green ≥0.5 / yellow [0.25, 0.5) / red with sine-pulse alpha <0.25) were completely untested despite being a core HUD signal; (b) `drawBulletTimeMeter` visual contract (fill width = barW × charge, ready-and-idle sine pulse, solid α=1 during active or refilling) had no direct tests — the I2 block tested state mechanics not the render output; (c) `updateShieldAura` pin-behaviour across multiple player-movement frames + exact pulse-alpha formula `0.75 + 0.25*sin(t*0.005)` — I4 tested one frame and single-call pin, not the multi-frame integration with movement; (d) bomb + boss composition (`activateBomb` reduces boss HP by 20, triggers `defeatBoss()` when lethal, leaves boss alive when non-lethal); (e) bomb × scoreMultiplier **design invariant** — bomb does NOT apply the multiplier (panic-button intentional design, locks against future "fix that oversight" edit); (f) danger warning one-shot — SFX fires once at 60% HEIGHT crossing, does not re-fire on subsequent ticks; (g) wave-complete clears both player and enemy bullets via `clearAllBullets`; (h) wave achievement stacking at wave 20 (WAVE_5 + WAVE_10 + WAVE_20 all fire); (i) `checkGameOverConditions` no-op during `waveTransitioning`; (j) bullet-time time-scale (BULLET_TIME_SCALE=0.3) demonstrably shrinks enemy advance distance by exactly that factor — the I2 test block asserted charge state but never that the scale actually propagates through `updateEnemies`. Regression tripwire: **23 new tests** in `GameScene.test.ts § R85.I9 Integration coverage refresh` split across 6 sub-describes — `drawHealthBar colour thresholds` (4), `drawBulletTimeMeter fill behaviour` (5), `updateShieldAura integration` (3), `Bomb + boss integration` (4), `Danger warning one-shot` (3), `Wave-complete + transitioning integration` (3), `Bullet-time time-scale` (1). Tests inspect mock fillStyle/setAlpha/fillRect call logs + direct state assertions. Minor snag during shakedown: `playSound('danger_warning')` vs `'dangerWarning'` — `SOUND_KEYS.DANGER_WARNING = 'dangerWarning'` (camelCase, not snake) — fixed in-test. All 2731/2733 unit tests green (2 pre-existing todo, +23 net from R85.I9), 54 test files, tsc clean, lint clean (0 err, 4 pre-existing warnings unchanged), build green in 7.50s.

**Stream M — Metris polish (ship after Stream G, parallel with Stream I, ~5 iterations):**

- [x] **R85.M1 [P0]** **First-block-spawn position** — **SHIPPED 2026-04-21** (co-fixed with M2, single root cause). Tom: *"the first block appears near the bottom and requires quick reactions, it needs to appeaar at the top and fall down, like the rest of the blocks do"*. Root cause was NOT a spawn-position bug — `spawnPiece()` correctly initialises every piece with `y: 0` (confirmed by the new "all 7 tetromino types spawn at y=0" test). The real bug was a timer race: `create()` armed `this.startDropTimer()` on line 94 **before** calling `this.startCountdown(5, () => {})` on line 97. `update()` was gated with `if (this.isCountingDown) return;` but drop ticks are on a Phaser `time.addEvent` timer callback (`dropTick()`) that runs independently of `update()` and had no such gate. With drop speed `INITIAL_DROP_SPEED=400ms` at level 1 and a 5-second countdown window, ~12 drop ticks fired while the player was reading "5, 4, 3, 2, 1", pushing the first piece from `y=0` to `y≈12` on a 20-row grid — "near the bottom" from Tom's POV. Fix is two-pronged defence-in-depth: (a) `create()` no longer calls `startDropTimer()` directly — it's passed as the `startCountdown` `onComplete` callback so the first tick can't fire until "GO!" resolves; (b) `dropTick()` itself early-returns on `this.isCountingDown || this.isPaused` so any future refactor that rearms the timer pre-countdown cannot resurrect the bug. 9 new tests in `GameScene.test.ts § R85.M1 / M2 Countdown-gated drop timer` lock: dropTick no-op during isCountingDown, no-op during isPaused, resumes normally once countdown flips false, respects isGameOver independently, `create()` ordering invariant (startCountdown fires before startDropTimer inside its onComplete), `startDropTimer` NOT called when startCountdown never fires its callback (regression tripwire), 5-second arity on startCountdown matches arcade convention (Pong/Snake/Bird/Frogger/NeoJump/AgentChase/CodeBreaker/MatrixCloud/CloudJumper/MatrixInvaders all use 5s), all 7 tetromino types spawn at `y=0`, 12-unguarded-ticks pathological case (exact live-bug reproduction) confirms piece stays at `y=0`. All 2676/2678 unit tests green (+9 net from M1/M2), tsc clean, lint clean (0 err, 4 pre-existing warnings unchanged), build green in 7.56s.
- [x] **R85.M2 [P1]** **5-second countdown after menu** — **SHIPPED 2026-04-21** (co-fixed with M1). Tom: *"5-second countdown fires correctly after menu — nope... We need a 5 second countdown too."* The countdown text was actually firing correctly in code — `startCountdown(5, () => {})` was called in `create()` at line 97 with text painted centre-screen at depth 200 (above all gameplay layers at depth 2/3/10). Tom's perception of "no countdown" was a cascade symptom of M1: because the drop timer was racing with the countdown, the first piece visibly slid down the board DURING the countdown, which made the countdown feel "missing / broken" from a player POV — if gameplay is already happening, the countdown no longer reads as a pre-game moment. Once M1's fix pinned the piece at `y=0` for the full 5s, the countdown became mechanically meaningful again. No separate M2-specific code change needed — the single `startCountdown(5, () => this.startDropTimer())` swap gives both tasks their fix. The M1/M2 test block asserts the 5-second arity explicitly (`toHaveBeenCalledWith(5, expect.any(Function))`) so a regression to a shorter countdown or missing callback would fail. Gates all green as per M1.
- [x] **R85.M3 [P1]** **Bullet-time B key manual activation** — **SHIPPED 2026-04-21**. Root cause: `handleScoring` had a 3-line auto-trigger block — `if (!this.bulletTimeActive && this.bulletTimeMeter >= MAX) this.activateBulletTime();` — that fired the moment a line clear tipped the meter to full, stealing the activation moment from the player. The `tryManualBulletTime()` method already existed (properly gated on `meter >= MAX` + not-already-active) and was already wired to the B key via `if (this.bKey && Phaser.Input.Keyboard.JustDown(this.bKey)) this.tryManualBulletTime()` in `handleInput` at line 776, so the fix is simply deleting the auto-trigger block — no new code paths. Verified that the HUD teach surfaces were already in place: `meterLabel` text is `'BULLET TIME [B]'` from scene setup (advertises the verb), fill colour turns YELLOW at 100% (vs GREEN while filling, CYAN while active), and `bulletTimeTimerText` already writes `'READY!'` when full-but-idle. Small extra teach: `updateHUD` now also switches `meterLabel.setColor` to YELLOW_HEX when ready-but-idle (was always PRIMARY_HEX pre-fix) so label + meter fill + timer-text all three pulse yellow together — player cue is unmissable. Regression tripwire: 9 new tests in the `Bullet Time` describe block — (a) `handleScoring(4)` at meter=90 does NOT auto-activate (captures bug inversion), (b) `handleScoring(1)` at meter=MAX-1 does NOT auto-activate (single-line edge), (c) `tryManualBulletTime` at full meter DOES activate + drains meter + arms timer, (d) `tryManualBulletTime` no-ops when already active (protects against accidental re-entry), (e) `updateHUD` sets YELLOW_HEX when ready-but-idle, (f) `updateHUD` stays PRIMARY_HEX below threshold, (g) `updateHUD` writes READY! text when ready-but-idle, (h) source-level regex tripwire asserts `handleScoring` body contains no `activateBulletTime()` call (immune to mock quirks, catches a regression even if the logic test were bypassed), (i) source-level regex tripwire asserts the B-key JustDown handler invokes `tryManualBulletTime()` and nothing else. Replaced the pre-existing *"should auto-activate when meter reaches 100"* test (which codified the bug) with its inverse. All 126/126 Metris tests green, 1835/1835 Phaser game tests green, tsc clean, lint 0 err / 4 pre-existing warnings, build green in 7.78s.
- [x] **R85.M4 [P2]** **Column spacing on right and left** — **SHIPPED 2026-04-21**. Tom: *"The spacing of the columns on the right and left Needs a bit of improvement."* Audit of the 800×450 canvas found 5 concrete issues behind the vague feedback: (1) the right-side `panel_tall` backdrop was sized **80×240** — three times the height of the left `HOLD` preview panel at 80×80, so the right wing read as top-heavy while the left looked compact; (2) **50 px of dead air** between HOLD panel bottom (y=110) and the first stat label `SCORE` (y=160) — the only game-HUD gap in the arcade that large; (3) the `BULLET TIME [B]` block was orphaned with a **35 px gap** from `COMBO` while the other stat-row gaps were 25 px; (4) the control-hint font colour was the literal hex string `'#005500'` — darker than `MATRIX_COLORS.DARK_GREEN_HEX` and well below the arcade-wide `DREAD_GREEN_HEX='#007700'` ambient-text convention introduced in R83.CTRLS.17, so the 7 control hints on the right were barely legible; (5) loose stat/control text had no visual grouping — the HOLD/NEXT preview panels were the only backdrops in the whole HUD. Fix architecture centralises every layout y/width/height into a **named config block** in `Metris/config.ts` (15 new constants: `PREVIEW_PANEL_W/H/Y_OFFSET`, `STATS_Y_START/ROW_H/LABEL_VALUE_GAP`, `BULLET_TIME_LABEL_Y/BAR_Y/BAR_W/BAR_H/TIMER_Y`, `HIGH_SCORE_LABEL_Y/VALUE_Y`, `CONTROLS_Y_START/ROW_H`, `STATS_PANEL_W/H`, `CONTROLS_PANEL_W/H`) so layout intent lives as data, not magic numbers scattered across 30 lines of `this.add.text(…, 175, …)` calls. Specific moves: (a) NEXT preview panel 80×240→80×80 matching HOLD, (b) stats block starts at y=120 (10 px below HOLD panel bottom, was y=160 = 50 px gap), (c) uniform STATS_ROW_H=20 rhythm across SCORE/LEVEL/LINES/COMBO (was 25 px for top three and 35 px to BULLET TIME), (d) control hints recoloured to `MATRIX_COLORS.DREAD_GREEN_HEX`, (e) two subtle `panel_tall` backdrops at α=0.18 behind the left stats+bullet-time stack (108×156) and the right HIGH SCORE+controls list (120×142) give the eye a frame for each HUD block without competing with the primary focal points (previews, meter fill, value digits). `createHUD` now iterates `statLabels`/`ctrlLines` arrays rather than hand-placing every text node — shorter code and the y-rhythm invariant lives in one `forEach`. `drawBulletTimeMeter` uses `BULLET_TIME_BAR_W/H/Y` constants so the meter bar is centred under HOLD_X by construction (`barX = HOLD_X - BULLET_TIME_BAR_W / 2`) rather than the magic `HOLD_X - 60` it used pre-fix. Regression tripwire: **24 new tests** in `GameScene.test.ts § R85.M4 Column Spacing Layout` split into 6 sub-describes — (i) **Preview-panel symmetry** locks `PREVIEW_PANEL_W === PREVIEW_PANEL_H`, floors `PREVIEW_PANEL_H < 240` (the pre-R85.M4 bug value — direct regression tripwire), and asserts `HOLD_X`/`NEXT_X` are symmetric around the grid centre; (ii) **Left-column stats rhythm** asserts `STATS_Y_START` sits within 20 px of the HOLD panel bottom (no dead air), `STATS_ROW_H ∈ [12, 24]`, `STATS_LABEL_VALUE_GAP < STATS_ROW_H` (label-value tighter than between-rows); (iii) **Bullet-time block rhythm** asserts label-y gap from last stat is `STATS_ROW_H/2 .. 2×STATS_ROW_H`, bar is drawn centred under HOLD_X (reads `strokeRect.mock.calls` — catches regression from the magic `-60` offset), bar uses the config W/H; (iv) **Right-column hierarchy** asserts `HIGH_SCORE_LABEL_Y === STATS_Y_START` (matched horizontal rhythm across both wings), `CONTROLS_ROW_H < STATS_ROW_H` (controls are secondary), all 7 control rows fit within the controls panel; (v) **createHUD wiring** introspects `add.text.mock.calls` — stat labels painted in correct order at `STATS_Y_START + i × STATS_ROW_H`, 7 control hints all use a colour that regex-matches `#00[79a-f][0-9a-f]00` (refuses `#005500`), uniform `CONTROLS_ROW_H` rhythm, bullet-time label at exact `BULLET_TIME_LABEL_Y`, HIGH SCORE at exact `HIGH_SCORE_LABEL_Y`; (vi) **createGraphics panel symmetry** introspects `add.image.mock.results` — HOLD and NEXT preview panels both render at `PREVIEW_PANEL_W × PREVIEW_PANEL_H`, **no image anywhere in the scene renders at h=240** (direct tripwire for the pre-R85.M4 bug), secondary stats + controls panels use α ≤ 0.25 (subtle grouping, not competing). All 150/150 Metris tests green (+24 net), 2708/2710 full suite green (2 pre-existing todo), tsc clean, lint clean (0 err, 4 pre-existing warnings unchanged), build green in 7.35s. Visual baseline regen deferred to R85.V1 — the panel resize + text repositioning WILL invalidate the existing Metris baseline, which is the expected cost of the polish.
- [x] **R85.M5 [P1]** **Metris unit-test coverage refresh** — **SHIPPED 2026-04-21**. M1-M4 each shipped with dense per-task tripwires (+42 tests net across Metris). R85.M5 fills the **cross-cutting integration** niche — interactions between features where a seemingly-unrelated refactor could silently break a seam. Audit surfaced 7 concrete gaps behind the existing 150 tests: (a) **drop-speed modifier compounding** — `getEffectiveDropSpeed` has two conditional modifiers (soft-drop overrides base speed, bullet-time divides by SLOWDOWN) but no test asserted the **compound order** (soft-drop + bullet-time simultaneously); pre-M5 coverage only tested each modifier in isolation, so a regression that swapped the order or added a short-circuit could silently halve the bullet-time effect; (b) **bullet-time lifecycle side-effects** — `activateBulletTime`/`deactivateBulletTime` both call `restartDropTimer` as a side-effect because drop speed changes; also `activateBulletTime` persists `bulletTimeCount` into save-system preferences (so players see their total across sessions); also `update()` decrements the timer by `delta` and auto-deactivates at ≤ 0 — none of this integration behaviour was directly tested; (c) **line-clear → level-up → drop-timer cascade** — `handleScoring` advances the level when `lines / LINES_PER_LEVEL + 1 > level`, which calls `restartDropTimer` to pick up the new faster speed; pre-M5 coverage tested level advancement and drop-speed-by-level independently but never the **cascade** from single method call to multi-side-effect; (d) **bullet-time meter saturation** — `handleScoring` clamps meter at `BULLET_TIME_MAX_METER` via `Math.min`, but no test pushed the boundary by stacking multi-line clears over the cap (90 + 4×20 would overshoot by 70 without the clamp); (e) **hold × bag integration** — first hold (no stored piece) consumes from the 7-bag via `nextPieceType`, subsequent holds swap without consuming — the bag side-effect wasn't directly asserted; also `canHold` resets to `true` after `lockCurrentPiece` (re-enabling hold for the next piece) wasn't tested as an integration; (f) **save-system persistence round-trip on game-over** — `handleGameOver` writes `highScore + level + stats` via `updateGameSave('metris', {...})` with stats-merge semantics (`gamesPlayed += 1`, `totalScore += score`, `bestCombo = max`, `longestSurvival = max`) — none of this contract was tested (pre-M5 only tested `this.highScore = Math.max(...)` in-memory, not the persistence call); (g) **`update()` gate asymmetry** — pause + game-over both expose test state but skip input handling; countdown early-returns **before** exposing state (so E2E harnesses don't see mid-countdown gameplay flicker). Regression tripwire: **23 new tests** in `GameScene.test.ts § R85.M5 Integration coverage refresh` split across 6 sub-describes — `Drop-speed modifier compound` (3 tests including the exact `Math.round(SOFT_DROP_SPEED / BULLET_TIME_SLOWDOWN)` compound-formula assertion + strict-monotonicity tripwire), `Bullet-time lifecycle side-effects` (4 — both activation paths call restartDropTimer, save-system persistence with prefs merge, update() decrement + auto-deactivate), `Line-clear → level-up → drop-timer cascade` (4 — threshold-crossing triggers restart, below-threshold does NOT, meter saturation at MAX, tetris-specific shake/flash args), `Hold × bag integration` (3 — first hold pulls from bag, swap hold does NOT, canHold resets post-lock), `Game-over save-system persistence` (3 — updateGameSave payload shape, stats-merge max semantics, double-game-over no-op), `update() gate semantics` (3 — paused exposes state/skips input, gameOver exposes state/skips input, countdown skips state exposure entirely), `getTestState contract` (3 — countdownValue present, ghostY nullable with null piece, pieceBagSize mirrors live bag). Minor snag during shakedown: the update() timer-decrement test tripped on `Phaser.Input.Keyboard.JustDown` not being defined in that specific test context because the global Phaser namespace wasn't hoisted into the rebound prototype closure — fixed by stubbing `scene.handleInput = vi.fn()` before calling update() (same pattern as gate-semantics tests). All 173/173 Metris tests green (+23 net from R85.M5, 42 net total across M1-M5), 2754/2756 full suite green (2 pre-existing todo), tsc clean, lint clean (0 err, 4 pre-existing warnings unchanged), build green in 7.71s.

**Stream V — Verification + Baselines (ship last, 1 iteration):**

- [x] **R85.V1 [P1]** **Visual baseline regen + full gate battery** — **SHIPPED 2026-04-21**. Full seven-gate battery green on darwin host: (1) `npx tsc --noEmit` — exit 0, no diagnostics; (2) `npm run lint` — 0 errors, 4 pre-existing warnings unchanged (KeyPanel fast-refresh, useSoundSystem × 2 exhaustive-deps, PhaserGame exhaustive-deps — all documented baseline noise from R78+); (3) `npm run build` — 7.64s, 1955 modules transformed, 292 PWA precache entries, no dist/ regressions; (4) `npm test -- --run` — **2754 / 2756 passed (2 pre-existing todos)** across 54 files in 14.65s (net +206 regression tripwires from R85.G1 7 + R85.G2 4 + R85.I1 4 + R85.I2 9 + R85.I3 9 + R85.I4 14 + R85.I5 11 + R85.I6 24 + R85.I7 24 + R85.I8 17 + R85.I9 23 + R85.M1/M2 9 + R85.M3 9 + R85.M4 24 + R85.M5 23); (5) `npx playwright test --project=chromium --update-snapshots` — **74 / 77 passed in 4.3 min**, 3 skipped (snake-classic frame-rate budget spec + the two CTRL-S walkthrough-capture specs which are known `.skip()` decorators, not failures). No logic regressions. (6 + 7) **Baseline refresh**: interesting observation from the gate run — existing darwin baselines (Apr 13, pre-R85) absorbed every R85.I1/I7/M4 visual change within the generous `maxDiffPixels: 200000 / threshold: 0.5` tolerance (playwright.config.ts:108-117 documents the structural-regression-not-pixel-identity intent, so this is by design). To keep the on-disk baseline pngs *visually accurate* representations of the current R85-polished build (rather than stale April-13 pig-enemy + 80×240-right-panel art) the darwin pngs for Invaders + Metris were manually deleted + regenerated via `npx playwright test --project=chromium --update-snapshots=missing e2e/playthrough/matrix-invaders.spec.ts e2e/playthrough/metris.spec.ts`. File-size deltas confirm meaningful content change: `matrix-invaders-05-paused` 44k → 267k (+6× — captures new bullet-time meter + power-up legend + scanline overlay HUD layers), `metris-03-early-play` and others similarly updated. Spot-checked two baselines with Read tool: Invaders menu shows R85.I5 hint-above-button layout + `B: Bullet Time` advertisement (I2); Metris early-play shows M4 HOLD/NEXT 80×80 symmetric panels (was asymmetric 80×80 + 80×240 pre-M4), compact stats rhythm under HOLD (was 50-px-dead-air pre-M4), `BULLET TIME [B]` teach surface (M3), DREAD_GREEN control hints (M4). Re-run of the two specs against the fresh baselines: 4/4 pass in 1.0 min, confirming the new snapshots are stable. Linux baselines (`-chromium-linux.png`) left untouched — those are produced by Docker CI (playwright.config.ts comment: "baselines must be regenerated inside Docker... to avoid host drift"); regen there is a separate Dockerfile.playwright task + the `-chromium-darwin` / `-chromium-linux` convention keeps the two platforms from colliding. Net files touched: 12 darwin pngs (6 Invaders × 6 Metris). Commit includes regen + plan tick + this ship note.
...
- **Game-load snap transition still sub-optimal (R83.G5 survives, polish bucket)**: `manual-testing-sessions/MANUAL_TESTING_CHECKLIST_Metris.md:18` flags the snap-in as "can be improved" and explicitly asks for this to be logged here for CI-bucket-style polish post-R85. R83.G5 already ships an `IpodBootOverlay` CRT-boot mask (`src/components/ui/IpodBootOverlay.tsx` — 450 ms phased reveal, sits inside the 500 ms `GamePortal` mount window) so the mechanism works — Tom's "survives but sub-optimal" reads as "the mask hides the flicker but the reveal itself could land with more Matrix-style impact". Candidate polish levers (for a future CI-bucket iteration, NOT R85 scope): (a) extend the scanline-wipe phase from ~150 ms to ~220 ms so the eye registers the CRT sweep before gameplay appears; (b) add a brief green-phosphor bloom at t≈350 ms that decays over 100 ms; (c) synchronise the overlay's audio sting (currently silent) with the reveal to compound the transition's weight. Scope when promoted: 1 iteration, single file `IpodBootOverlay.tsx` + existing Vitest harness `IpodBootOverlay.test.tsx` to lock the new timings.
