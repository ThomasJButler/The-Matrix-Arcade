...
## Status: R82 open — Round 2 playtest refinements: R82.21–R82.28 [x] (portal top-clip + 16:9 full-fill + dashbar redesign + EXIT vs PAUSE split + Trophy icon on clickwheel bottom + Key panel right-of-iPod + bigger PLAY button on both clickwheel centre and dashbar centre + visual regression baselines regenerated). R82.29 (manual human playtest) still unchecked — Tom to action. R82.13 continuous-improvement resumed: sticky-hover fix on mobile — 4 hover rules (.clickwheel-zone, .clickwheel-centre, .dashbar-btn, .dashbar-centre) split so `:hover` is gated behind `@media (hover: hover) and (pointer: fine)` while `:focus-visible` stays unconditional; prior to this, a tap on a touchscreen left the button stuck in its hover animation (infinite `clickwheel-pulse`/`clickwheel-pulse-centre`) until the user tapped elsewhere. Desktop chromium still matches the media query so visual baselines remain byte-identical.
