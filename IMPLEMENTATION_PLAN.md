...
- [ ] **R83.G7 [P2]** Testing-doc template upgrade for the 8 untested games — apply the "Known Issues (carried forward) — links to R83" header block that Tom added to the 4 tested docs to `MANUAL_TESTING_CHECKLIST_{Agent_Chase,Cloud_Jumper,Code_Breaker,Matrix_Frogger,Matrix_Invaders,Metris,Neo_Jump,Rhythm_Hacker}.md`. Header should include: severity legend, "test against current commit", and a pointer back to the global known-issues section so Tom isn't re-writing them per file. Pure markdown task, one iteration.
