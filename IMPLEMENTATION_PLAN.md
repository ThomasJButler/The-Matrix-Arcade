...
## Status: **R86 open** (2026-04-22 late — Rhythm Hacker R1+R2 P0 blockers shipped). **Stream F all shipped + Tom-signed-off** (*"im happy with matrix frogger, its good"* 2026-04-22 — F6 ticked). **Stream G all shipped**. **Stream N N1-N4 shipped**; N5 **bumped P2→P1** per Tom's flow-broken playtest (*"still too difficult... can't get momentum... 5s restart too heavy"*). **Stream A (Agent Chase) ADDED 2026-04-22** — lifted from R87 scope per Tom's faster test cadence. A1 scoreboard integration (P0 — 3rd game with save-path gap), A2 middle-box ghost-release bug, A3 coverage. R87 now covers final 3 games only (Rhythm + CloudJumper + CodeBreaker). Remaining R86 work: **R1/R2 Rhythm Hacker P0 unblock + N5 Neo Jump flow + A1 scoreboard live-refresh (P0 → P1 after Tom's 2026-04-22 clarification — scores DO persist on refresh, bug is read-path / modal-stale-snapshot) + A2/A3 Agent Chase + R3/R4 Rhythm Hacker polish + V1 baselines**. Loop cap: `./loop.sh 15`. R87 now covers final 2 games only (CloudJumper + CodeBreaker). Loop-sentinel rule preserved. **R85 awaiting sign-off** (Tom-tick only — phrase lives in R85 stub below). R84.S7 `[DEFER]` + R84.CI bucket + R83.CTRLS umbrella also Tom-tick only. Full R85 archive at [`COMPLETED_WORK.md § R85`](COMPLETED_WORK.md#r85--invaders--metris-polish--2-globals-2026-04-20--2026-04-21).
...
- [ ] **R86.A1 [P1]** **Scoreboard modal live-refresh gap** (SCOPE REVISED 2026-04-22 per Tom's clarification) — **bumped DOWN P0 → P1**. Original diagnosis was wrong: the WRITE path works (Tom confirms *"this does work, with page refresh"* for both Neo Jump AND Agent Chase — R86.G1's defensive `updateGameSave` on Neo Jump shipped correctly, and Agent Chase scores persist too). The actual bug is the **Scoreboard modal reads a stale snapshot mid-session** — new scores don't appear until a page refresh reloads the store from localStorage. Classic Zustand/React reactive-subscription gap: either the high-score write bypasses the Zustand setter (direct localStorage write without notifying subscribers), or the Scoreboard modal component reads from a closure / one-time `getState()` call instead of subscribing via `useStore`. **Fix approach**: (a) audit `useSaveSystem`'s `updateGameSave` path — ensure every write goes through the Zustand store's setter (so `subscribe` listeners fire), not directly to `localStorage`; (b) audit `Scoreboard` modal + `AttractMode` + iPod dashbar trophy — ensure they use `useStore((state) => state.games)` hook form, not a one-shot `useStore.getState()` read. Fix whichever end is broken. Add regression test asserting: after a game-over that sets a new high score, the Scoreboard modal (opened WITHOUT refresh) shows the new value within one render cycle. **Defensive Agent Chase `updateGameSave` on death** — still worth adding as belt-and-suspenders (keep the original R86.A1 plan as a sub-commit), pattern-match with R86.G1. **Cross-game audit**: Rhythm Hacker + CodeBreaker + CloudJumper likely affected by the same read-path bug; grep for one-shot `useStore.getState()` reads in their scoreboard-facing components while fixing. Pre-emptive fix here likely resolves R87 scoreboard complaints before R87 even opens.
