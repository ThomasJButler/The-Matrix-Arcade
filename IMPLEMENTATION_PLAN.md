...
## Status: R82 open — R82.13 continuous-improvement: visual detailing — ambient radial halo behind the iPod body. Addresses R82.20 discovered-work finding *"Portal vertical whitespace feels 'floaty' on 900px+ viewports"*. Added `isolation: isolate` to `.game-portal-container` to create a stacking context, then `::before` pseudo at `z-index: -1` paints a soft matrix-green radial gradient (ellipse 55%×45%, 9%→4%→1.5%→transparent) that "grounds" the device — reads as implicit floor/ambient bloom without being visually loud. The `isolation` is load-bearing: without it, a negative z-index can escape the positioned ancestor and render behind the entire app root instead of just behind sibling content. Pure CSS, static (no keyframes), zero repaint cost, implicitly reduced-motion-safe. The container's existing `overflow: hidden` clips the halo at the portal bounds so it never bleeds into header/footer. No JSX change — scoped entirely to `src/styles/theme.css:453-482`. Gates: lint 0 errors / 10 intentional warnings, 23/23 unit tests (GamePortal + App), build clean (7.14s). Ralph does NOT write terminator — Tom's manual call post-review.
