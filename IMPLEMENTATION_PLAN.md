...
## Status: **R86 open** (2026-04-21 — Stream G wrapped). R86 = **Frogger + Neo Jump polish + 3 global regressions** (Batch 2 of the 3-batch per-game polish cadence started by R85). Loop cap: **20 iterations**. **Stream G all `[x]`** — G1 Neo Jump save-path (mirrors Metris belt-and-suspenders pattern, +3 tests), G2 click-to-focus traced to React 17+ SyntheticEvent.stopPropagation on the dashbar rove handler killing Phaser's native window keydown (fix reroutes `handlePlayPress` focus onto a `'game'` target that a `MutationObserver` lands on `[data-phaser-game="true"]`, +3 tests), G3 WebGL warnings promoted arcade-wide — R83.CTRLS.11's `premultipliedAlpha: false` + `mipmapFilter: ''` hoisted into a shared `PHASER_RENDER_DEFAULTS` constant in `src/lib/phaser/types.ts`, spread into every game's render block (12 games: AgentChase, CloudJumper, CodeBreaker, CtrlSWorld refactored to use shared constant, MatrixCloud, Frogger, Invaders, Metris, NeoJump, RhythmHacker, Snake, VortexPong). New cross-game regression tripwire at `src/lib/phaser/render-config.test.ts` enumerates all 12 configs + asserts both keys present — future game additions that forget the spread fail the gate. +14 regression tests (**2774 total + 2 todo**). Stream G shipped means Streams F + N are unblocked. **Next critical = R86.F1 Frogger finish-line crash** — only Level 1 playable until that lands. Loop-sentinel rule preserved (no `\bCOMPLETE\b` on Status line mid-phase — use "shipped" / "wrapped" / "all `[x]`"). **R85 awaiting sign-off** (Tom-tick only — terminator phrase lives in the R85 stub below). R84.S7 `[DEFER]` + R84.CI bucket also Tom-tick only. R83.CTRLS umbrella pending Tom's CTRL-S end-to-end tick. Full R85 archive at [`COMPLETED_WORK.md § R85`](COMPLETED_WORK.md#r85--invaders--metris-polish--2-globals-2026-04-20--2026-04-21).
