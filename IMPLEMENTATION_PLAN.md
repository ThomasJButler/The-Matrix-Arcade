...
## Status: R82 open — Round 2 playtest refinements: R82.21–R82.28 [x]. R82.29 (manual human playtest) still unchecked — Tom to action. R82.13 continuous-improvement: a11y — **Windows High Contrast Mode / macOS Increase Contrast support**. New `@media (forced-colors: active)` block in `animations.css` (placed alongside the existing reduced-motion block at line ~1253) opts the iPod chrome IN to the user's system palette via `Canvas`/`CanvasText`/`ButtonBorder`/`ButtonText`/`Highlight`/`GrayText`/`Mark`/`MarkText` rather than opting out via `forced-color-adjust: none`. Why this matters: HCM strips `box-shadow`, background gradients, and most colour backgrounds, and the entire iPod visual language (device bezel, screen inset, clickwheel ring, dashbar pill, four wedge separation) was encoded in those strips — with HCM active the whole portal previously collapsed into an indistinguishable flat block where the 4 clickwheel quadrants + centre were visually one button. Fix adds explicit 1-px `ButtonBorder` borders around each of the 5 wheel zones + the 3 dashbar buttons so hit-zone geometry survives the theme swap; 1-px `CanvasText` borders around `.ipod-body`, `.ipod-screen`, `.ipod-clickwheel`, `.ipod-dashbar` so the device chrome edges don't vanish; `outline: 2px solid Highlight; outline-offset: 2px` on all focus-visible states (wheel toolbar + dashbar toolbar + every zone + centre + dashbar buttons) so keyboard focus contrasts with whatever theme the user picked; `GrayText` on `:disabled` (prev/next/scores while playing, play while transitioning) so the disabled affordance speaks the idiom users' OS theme already taught them to recognise. Pause overlay (which normally uses `backdrop-filter: blur` + amber scrim that HCM nulls) now anchors on `Canvas` with a 2-px `CanvasText` border and `CanvasText` text so PAUSED + "PRESS P OR ❚❚ TO RESUME" remain legible once the amber glow is gone. Mute pill uses the CSS Color Level 4 `Mark`/`MarkText` pair — the system-reserved highlight/alert colour — since the pill's semantic is "audio-disabled warning"; themes that don't support Level 4 simply fall through to the existing red-pill declarations, no visual regression. **Why opt-in over opt-out**: `forced-color-adjust: none` would force our green-on-black matrix palette regardless of what HCM chose (Windows has at least 4 stock HCM themes: High Contrast Black, High Contrast White, High Contrast #1, #2 — plus custom), ignoring the user's explicit accessibility choice. Opting in via system colour keywords respects the theme the user deliberately picked while we still handle the LAYOUT preservation (borders where shadows used to imply edges). **Why not `prefers-contrast: more` in this iteration**: different concern — `prefers-contrast: more` is "user wants more contrast ON TOP OF author's palette" (still sees our greens, just bolder), while `forced-colors` is the hard-substitution path. Per the one-improvement-per-iteration rule, `forced-colors` is the stronger gap (affects WCAG 1.4.3 contrast + 2.4.11 focus-not-obscured for keyboard users on HCM themes) and ships alone. Zero JS changes — pure CSS, no new keyframes, no new motion work needed. Zero test churn: forced-colors media queries can't be emulated in jsdom without `matchMedia` trickery; the block is declarative enough that a snapshot-style test would just mirror the CSS. Zero visual regression: chromium visual runners don't enable HCM, so Darwin + Linux baselines stay byte-identical — confirmed by the 67/67 App + GamePortal + KeyPanel test pass (no snapshot warnings). `@media (forced-colors: active)` is supported across Chrome 89+, Firefox 89+, Safari 17.4+, Edge 79+ — no feature gate or progressive-enhancement fallback needed. **Gates all green**: lint 0 errors / 11 pre-existing warnings, tsc clean, 67/67 App + GamePortal + KeyPanel pass, build clean (9.18 s). **Priority ordering**: stays in priority-1 accessibility. Previous iterations shipped the discoverability side (aria-keyshortcuts + aria-roledescription, debounced announcements), focus side (focus-visible polish, focus survives wheel↔dashbar crossfade), and motion side (prefers-reduced-motion). This iteration closes the theme side of a11y — HCM + macOS Increase Contrast users now see a usable portal instead of a collapsed block. Previous iterations (aria-keyshortcuts, debounce, idle-prefetch, hover-glow radial gradient, jump-nav wheel spin) still deployed.
