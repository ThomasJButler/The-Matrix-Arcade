...
- [ ] **R84.UP1 [P1]** Upstream pause-overlay investigation — **Stream A audit D3**: `BaseScene.togglePause` is single-owner (no duplicate overlay in scene code), yet Tom reports triple-stacked overlays across all games. Root-cause MUST live upstream — in `GamePortal.tsx` or `App.tsx`. Hunt: (a) grep for any `<PauseOverlay>`, `aria-label*="pause"`, or Tailwind class with `pause` in the portal/app tree, (b) live-inspect DOM during a paused game via DevTools — count overlay layers; if >1, trace to source. Fix to single source of truth (keep the R82 dashbar-amber scrim, remove any duplicate renderer). If after live inspection only ONE overlay shows, close as "stale complaint, code correct" and tick the 3 games' testing-doc pause-overlay lines.
