...
## Status: **R85 open** (2026-04-20 — Tom's playtest notes on Matrix Invaders + Metris filed). R85 = **Invaders + Metris polish + 2 NEW globals** carved out of the originally-planned 8-game polish phase because Tom wants to test the remaining 6 games first before committing to their scope. Loop cap: **20 iterations**. **Stream G globals — both shipped 2026-04-21** (G1 high-score persistence + G2 dashbar trophy). **Stream I kick-off 2026-04-21**: I1 enemy sprite redesign wrapped — procedural UFO/battleship silhouettes replace the face-like fallback sprites Tom flagged as "pigs". Shapes drawn in pure white so `ROW_TINTS` multiplies cleanly per row (previous green-base zeroed non-green tint channels), with bespoke silhouettes per type: `code` flying saucer, `agent` twin-pod battleship, `sentinel` hex cruiser with corner spikes, `virus` crystalline spike cluster. `spriteMode` check dropped for enemies — PNG fallback path retired since the 16px dump sprites read as blobs at 32x24. `setScale(0.8)` at spawn closes PG6's long-pending 20% shrink (virus split children inherit the same contract). Regression tripwire: 4 new tests in `GameScene.test.ts` pin texture-key contract, per-row tint bijection, shrink invariant + virus-child inheritance (2553/2555 unit tests green, 2 todo). Next: R85.I2 bullet-time HUD visibility. **R84 still awaiting sign-off** (R84.S7 `[DEFER]`, R84.CI bucket Tom-tick-only). R83.CTRLS umbrella also still pending Tom's CTRL-S end-to-end tick.
