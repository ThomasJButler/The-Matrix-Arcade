...
## Status: **R86 open** (2026-04-21 — Stream F mid-flight, F1 + F2 shipped). R86 = **Frogger + Neo Jump polish + 3 global regressions** (Batch 2 of the 3-batch per-game polish cadence started by R85). Loop cap: **20 iterations**. **Stream G all `[x]`** — G1 Neo Jump save-path, G2 click-to-focus (React 17+ SyntheticEvent.stopPropagation on dashbar rove handler), G3 WebGL warnings arcade-wide via shared `PHASER_RENDER_DEFAULTS` + cross-game regression tripwire. **Stream F R86.F1 `[x]`** — Frogger finish-line crash root-caused as a three-way tween/physics race; fix in guarded `triggerLevelUp()` with `isLevelingUp` flag + defence-in-depth on `handleEnemyCollision`/`handleInput`. **Stream F R86.F2 `[x]`** — countdown "missing" diagnosed as the same cascade R85.M2 fixed for Metris: `create()` pre-spawned enemies/pills + armed 2s/3s interval spawn timers BEFORE `startCountdown`, so the countdown digit painted at depth 200 but enemies were already on the board → Tom read the pre-game beat as "already in gameplay". Fix mirrors R85.M2 pattern: all gameplay arming funnelled through a new `armGameplay()` method passed as `startCountdown`'s onComplete (clean lanes-only board during the 5s window, first wave spawns the instant control returns); defence-in-depth on `spawnEnemy()` + `spawnPills()` with early-return on `isCountingDown || isGameOver || isLevelingUp` so a future refactor re-arming the timer pre-countdown cannot resurrect the bug. +9 regression tests in `R86.F2 — Countdown-gated spawn arming` describe block (each guard variant × 2 spawners, `armGameplay` spawn+timer arming, early-return on isGameOver edge case, end-to-end countdown flip tripwire). **2791 tests pass + 2 todo** (+9 from R86.F1's 2782). Gates green: tsc clean, lint 0 err, build 7.76s. Tom-tick awaited on next playtest — countdown should now read as a distinct pre-game moment on a clean board. **Next tasks**: R86.F3 (Kung Fu HUD clip), R86.F4 (vehicle hit-box audit), plus parallel-safe Stream N (Neo Jump polish). Loop-sentinel rule preserved (no `\bCOMPLETE\b` on Status line mid-phase). **R85 awaiting sign-off** (Tom-tick only). R84.S7 `[DEFER]` + R84.CI bucket + R83.CTRLS umbrella also Tom-tick only. Full R85 archive at [`COMPLETED_WORK.md § R85`](COMPLETED_WORK.md#r85--invaders--metris-polish--2-globals-2026-04-20--2026-04-21).
