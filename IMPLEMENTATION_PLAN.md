...
## Status: **R86 open** (2026-04-21 — Stream F mid-flight, F1 + F2 + F3 + F4 shipped). R86 = **Frogger + Neo Jump polish + 3 global regressions** (Batch 2 of the 3-batch per-game polish cadence started by R85). Loop cap: **20 iterations**. **Stream G all `[x]`**. **Stream F F1/F2/F3/F4 `[x]`**. **R86.F4 `[x]`** — vehicle hit-box encroachment root-caused as origin-geometry, not sprite scale: enemies render with `setOrigin(0.5, 1)` so their bottoms sit at `rowToY(row)` (lane centre) for the 3D lean, which means Phaser's `body.setSize(center=true)` anchors the body on the sprite's DISPLAY centre — `displayHeight/2` above the lane centre. For cars at row 5 (frame 16px × baseScale 3.0 × perspScale 0.85 → display 40.8px, laneH 66.6px) that lands `body.top` 7.5px past row 5's top, inside the row 4 middle safe zone. Agents/sentinels at 64×64 do the same into row 0 (finish) and rows 1-2. Fix extracts body sizing into a new `applyEnemyBody(enemy, row)` helper: (a) shrinks body width to `HITBOX.WIDTH_RATIO = 0.75` for lateral forgiveness, (b) caps body height at `HITBOX.HEIGHT_RATIO = 0.85` of `laneH[row]` (in world pixels, converted back to source via `/scaleY`), (c) re-offsets via `body.setOffset(offsetX, frame.height - bodyHeight/2)` so the body re-centres on `rowToY` rather than the displaced sprite centre. Chasers are exempt (they cross lanes via `verticalSpeed` — shrinking them to a single lane would break mid-traversal collision). New `HITBOX` config block in `config.ts` documents the why so future asset-pack swaps don't silently regress the geometry. +6 regression tests in a new `R86.F4 — Hit-box clamped to lane bounds` describe block: lateral-shrink floor (width < frame.width), row 5 car never extends past row 4 (Tom repro tripwire — derives `bodyTopRel` from the Phaser body-offset formula), body re-centres on rowToY (lane centre), height clamp triggers when lane is tight, chaser keeps full-frame body + no setOffset, HITBOX constants bounded in (0.5, 1]. Gates: tsc clean, lint 0 err (4 pre-existing warnings), **2803 tests pass + 2 todo** (+6 from R86.F3's 2797), `npm run build` clean 7.33s. Tom-tick awaited on next playtest — cars should no longer phantom-hit Neo while resting in the middle safe zone. **Next tasks**: R86.F5 (menu legend), R86.F6 (multi-level polish, post-F1), R86.F7 (test-coverage refresh), parallel-safe Stream N (Neo Jump polish). Loop-sentinel rule preserved (no `\bCOMPLETE\b` on Status line mid-phase). **R85 awaiting sign-off** (Tom-tick only). R84.S7 `[DEFER]` + R84.CI bucket + R83.CTRLS umbrella also Tom-tick only. Full R85 archive at [`COMPLETED_WORK.md § R85`](COMPLETED_WORK.md#r85--invaders--metris-polish--2-globals-2026-04-20--2026-04-21).
