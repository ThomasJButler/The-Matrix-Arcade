...
- [ ] **R87.K1 [P0]** **Green-bomb power-up soft-lock** — Tom's 2026-04-22 playtest: *"I tried to get the shooting power-up and I missed the last ball, but the power-up activated and allowed me to shoot bricks, but there wasn't another ball on the screen. So now the game is still playing, but there's no ball."* Screenshot `codebreakerbug.png` confirms: paddle + bricks + 2 lives + 0 balls + no game-over screen. Audit the green-bomb power-up flow: (a) pickup handler fires `activatePowerUp('bomb')` even if no ball is live, (b) bomb effect completes without checking post-state. Invariant to add at the end of every ball-consumer / explosion-consumer: `if (balls.length === 0 && lives > 0 && !isGameOver) respawnBall(); else if (balls.length === 0 && lives === 0 && !isGameOver) triggerGameOver();`. Consider whether power-up pickup itself should early-return when `balls.length === 0` (power-up orphaned → auto-discard). Unit tests locking both paths + regression test simulating "miss last ball + grab bomb" scenario.
- [ ] **R87.K2 [P0]** **Green-bomb spontaneous game-over mid-level** — Tom: *"I just found another glitch when I got the green bomb, and it closed the game, just terminated with a high score. I was almost finished level 1 when this happened."* Likely shares a root cause with K1 (bomb effect walks over the lives counter or trips an `isGameOver` flag from within its tween onComplete). If audit confirms same root cause, batch K1+K2 in a single commit; if different, split. Unit tests lock that `triggerGameOver()` is only called from explicit death paths (0 lives + 0 balls OR manual exit), never from a power-up onComplete.
- [ ] **R87.K3 [P0]** **Keyboard + numpad controls dead** — Tom: *"keyboard controls all respond - NOPE, only mouse controls work - keyboard controls dont do anything"* + *"Numpad keys work (Code Breaker specifically) - nope"* + *"Alt keys (WASD vs arrows) both work - nope"* + *"No keys get 'stuck' on rapid inputs - keyboard keys get stuck and cannot move unless use the mouse"*. Audit `CodeBreaker/scenes/GameScene.ts` `create()` — compare keyboard wiring against Pong/Metris/Snake. Likely missing `this.input.keyboard.addKeys(...)` OR the paddle is driven by `pointer.x` with no keyboard branch OR a mouse-mode flag is pinned-on. Add full keyboard branch: ←/→ + A/D + Numpad 4/6, with acceleration/deceleration matching mouse feel. Regression test: stub keyboard input, assert paddle.x changes under keydown.
