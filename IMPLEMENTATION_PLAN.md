...
## Status: **R86 open** (2026-04-21 — Stream F unblocked, F1 shipped). R86 = **Frogger + Neo Jump polish + 3 global regressions** (Batch 2 of the 3-batch per-game polish cadence started by R85). Loop cap: **20 iterations**. **Stream G all `[x]`** — G1 Neo Jump save-path, G2 click-to-focus (React 17+ SyntheticEvent.stopPropagation on dashbar rove handler), G3 WebGL warnings arcade-wide via shared `PHASER_RENDER_DEFAULTS` + cross-game regression tripwire. **Stream F R86.F1 `[x]`** — Frogger finish-line crash root-caused as a three-way tween/physics race: `movePlayer()` set `playerRow = 0` before its hop tween started so `checkProgress()` spun up a second tween on `this.player`, the physics body kept interpolating through road lanes 1-7 during the 300 ms reset and fired overlap → `playerDeath`, whose own tween then competed and stalled onComplete (Tom's "freezes and crashes"). Fix: extracted level-up into a guarded `triggerLevelUp()` with `isLevelingUp` state flag — kills in-flight player tweens, disables the physics body for the duration of the reset, belt-and-suspenders guards on `handleEnemyCollision` + `handleInput` + `bufferedInput` cleanup + `isMoving` reset, re-arms on reset-tween onComplete. +8 regression tests in `Level-up safety (R86.F1)` describe block (flag set, tweens killed, body disabled, re-entry blocked, collisions suppressed during level-up + game-over). **2782 tests pass + 2 todo** (+8 from R86.G3's 2774). Gates green: tsc clean, lint 0 err, build 7.73s. Level 2+ now reachable — Tom-tick awaited on next playtest. **Next tasks**: R86.F2 (5-second countdown after MenuScene), R86.F6 (multi-level polish — now unblocked by F1), plus parallel-safe Stream N (Neo Jump polish). Loop-sentinel rule preserved (no `\bCOMPLETE\b` on Status line mid-phase). **R85 awaiting sign-off** (Tom-tick only). R84.S7 `[DEFER]` + R84.CI bucket + R83.CTRLS umbrella also Tom-tick only. Full R85 archive at [`COMPLETED_WORK.md § R85`](COMPLETED_WORK.md#r85--invaders--metris-polish--2-globals-2026-04-20--2026-04-21).
