...
## Status: **R86 open** (2026-04-22 — Stream F mid-flight, F1 + F2 + F3 + F4 + F5 + F7 shipped). R86 = **Frogger + Neo Jump polish + 3 global regressions** (Batch 2 of the 3-batch per-game polish cadence started by R85). Loop cap: **20 iterations**. **Stream G all `[x]`**. **Stream F F1/F2/F3/F4/F5/F7 `[x]`** (F6 still blocked on Tom-tick — multi-level polish needs an actual Level 2+ playtest). **R86.F7 `[x]`** — P1 Frogger unit-test coverage refresh. The previous 99-test suite exercised ~every *behavioural* outcome of R86.F1–F4 but left several single-line *invariants* as implicit contracts: a removed `!isGameOver` short-circuit in `checkProgress`, a flipped early-return in `handleInput` when `isLevelingUp`, a missing `if (this.isCountingDown) return` in `update()`, or a dropped `applyEnemyBody` null-guard would pass lint and tsc yet silently reopen the exact classes of bug R86.F1/F2/F4 fixed. Fix adds **+11 regression tests** in three new describe blocks in `GameScene.test.ts` — all invariant-level tripwires rather than behaviour re-tests. (1) `R86.F7 — Finish-line guards` (×5): checkProgress short-circuits when `isGameOver` is already true (no level-up race after death), `handleInput` early-returns under `isLevelingUp` and under `isGameOver` (no ghost moves during the reset tween), `triggerLevelUp` fires exactly the F1 juice contract (`SOUND_KEYS.FROGGER_SCORE` + `cameras.main.flash(150, 0, 255, 0, false, undefined, undefined, 0.15)` — RGB literal locked so a future palette tweak fails gate), `showLevelUpText` renders `LEVEL 2` post-increment (guards ordering of `this.level++` vs text-render — off-by-one here would show `LEVEL 1` on first cross). (2) `R86.F7 — update() countdown gate` (×2): `stubUpdateLoopSpies` helper stubs every post-guard update method (`handleInput`, `updateEnemies`, `updatePowerUps`, `updateNeoFlash`, `applyMagnetEffect`, `updateCombo`, `checkProgress`, `exposeTestState`) so the test asserts zero calls when `isCountingDown=true` and exactly-one calls when false — complements F2's spawn-guard by locking the *top-level* `update()` early-return that F2 relies on. (3) `R86.F7 — Hit-box visible-bounds extensions` (×4): X-offset symmetry proves `applyEnemyBody` re-centres the body laterally (`offsetX === (frame.width - bodyWidth) / 2` — a stale origin here would slide the body off-sprite), 64×64 agent HEIGHT_RATIO clamp in tight upper lane (extends F4's 16×16 car coverage to the bigger sprite), body=null no-op (defensive guard preserved), frame=null no-op (ditto). Harness fix: `createTestScene()` previously omitted `isLevelingUp` and `bufferedInput` from its defaults — the real `FroggerGameScene` class field for each is initialised to `false`/`null`, so two tests failed with `undefined` mismatches; both defaults now seeded explicitly so `scene.isLevelingUp === false` and `scene.bufferedInput === null` hold identically to the class. No production code touched — this is a pure test-coverage refresh. Gates: tsc clean, lint 0 err (4 pre-existing warnings), **2832 tests pass + 2 todo** (+11 from R86.F5's 2821), `npm run build` clean 7.59s. F6 stays blocked on Tom's multi-level playtest; Stream N (Neo Jump polish) remains parallel-safe. **Next tasks**: R86.F6 (Tom-tick only), parallel-safe Stream N, Stream V verification last. Loop-sentinel rule preserved (no `\bCOMPLETE\b` on Status line mid-phase). **R85 awaiting sign-off** (Tom-tick only). R84.S7 `[DEFER]` + R84.CI bucket + R83.CTRLS umbrella also Tom-tick only. Full R85 archive at [`COMPLETED_WORK.md § R85`](COMPLETED_WORK.md#r85--invaders--metris-polish--2-globals-2026-04-20--2026-04-21).
