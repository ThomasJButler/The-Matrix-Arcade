...
## Status: R82 open — Round 2 playtest refinements: R82.21–R82.28 [x]. R82.29 (manual human playtest) still unchecked — Tom to action. R82.13 continuous-improvement resumed: clickwheel hover-glow calibration — flat-alpha hover/focus-visible paint (`rgba(0,255,0,0.05)` hover, `0.10` focus) replaced with a `radial-gradient(circle at 50% 50%, …)` anchored to the wheel's geometric centre, so the glow reads as LED backlight emanating from inside the disc rather than a flat wedge-flash. The geometry works cleanly because each `.clickwheel-zone` has `position: absolute; inset: 0` — its painting box covers the full wheel bounding square, so `50% 50%` lands on the real wheel centre; the existing `clip-path` polygon then reveals only the relevant wedge slice. Hover inner alpha 0.14 at 40% → 0.06 at 75% → transparent at rim; focus-visible strengthened a shade (0.18 → 0.08 → transparent) so keyboard focus still outranks hover, preserving the existing 0.85 → 1.0 colour ramp and the unchanged inset-box-shadow focus ring. `@keyframes clickwheel-pulse` updated in lockstep — 0% inner alpha 0.08, 50% inner alpha 0.20 — so the breathing animation shares the same geometric vocabulary; previously the keyframes flashed a flat background that jarred against the new radial focus paint. `:active` intentionally preserved with its sharp 0.14 flat-alpha + `scale(0.96)` depression — gradient paint on a tactile press would muddy the feedback. Also consolidated two cascading `.clickwheel-zone:focus-visible` rules into one: the earlier rule's `background` had already been overridden by the later rule (only its `animation` property survived the cascade), which was dead code masquerading as intent. Pulling both declarations into a single rule eliminates the cascade trap and makes the focus-visible surface readable top-to-bottom. **Why radial over flat**: the iPod Classic's click wheel is a capacitive ring — its visual language is "light emerging from inside the disc", not a painted wedge. Radial gradient matches that mental model; flat-alpha fights it. The change is invisible at rest (baseline snapshots stay byte-identical — hover/focus states aren't captured because the Playwright fixture never focuses/hovers before screenshot), and `@media (prefers-reduced-motion: reduce)` already disables `.clickwheel-zone` animations so the effect is automatically suppressed for users who've opted out. Separately: ticked the priority-list "Audio feedback" item ✅ — `playClick`/`playConfirm` were already wired to `scoreboardTab`/`scoreboardConfirm` across all 15 wheel/dashbar/keyboard handlers but the list never acknowledged it; retroactive hygiene. **Gates all green**: lint 0 errors / 11 pre-existing warnings, tsc clean, 58/58 App + GamePortal + KeyPanel tests pass, build clean (7.89s), visual baselines unchanged (rest-state paint untouched). Previous change (clickwheel rotation on jump-nav keys): still deployed; this iteration refines the hover/focus visual language.
...
- [ ] **R82.13 — [P3] ★ CONTINUOUS-IMPROVEMENT (never auto-checked, runs AFTER R82.14–R82.20 + R82.21–R82.29)** — each remaining loop iteration picks ONE micro-improvement from the priority list below and ships it. This task is **intentionally never marked `[x]`** — Tom manually ticks it when he's happy with the final result. Ralph MUST NOT write the terminator phrase to Status; the `loop.sh` iteration cap is the stop signal. Tom's call (2026-04-18): *"fuck it we can make it the best version we possibly can"*.
  ...
  **R82.13 micro-improvement priority list** (pick one per iteration, commit as `R82.13: <improvement summary>`):
  ...
  - **Perf micro-optimisations** — ~~`will-change: transform` on clickwheel segments~~ ✅ shipped 2026-04-16 (`.clickwheel-zone` gains `will-change: transform` matching `.dashbar-btn`; reduced-motion reset extended to release the new layer); ~~`contain: layout` where safe~~ ✅ shipped 2026-04-16 (`.ipod-clickwheel-region` gains `contain: layout style` to isolate wheel↔dashbar morph; `.ipod-loading-screen` gains `contain: layout style paint` to scope continuous Suspense-fallback animations); avoid layout thrashing in animations
