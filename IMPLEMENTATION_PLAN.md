...
## Status: R82 open — Round 2 playtest refinements: R82.21–R82.28 [x] (portal top-clip + 16:9 full-fill + dashbar redesign + EXIT vs PAUSE split + Trophy icon on clickwheel bottom + Key panel right-of-iPod + bigger PLAY button on both clickwheel centre and dashbar centre + visual regression baselines regenerated). R82.29 (manual human playtest) still unchecked — Tom to action. R82.13 continuous-improvement resumed: paused-state indicator shipped (PAUSE_STATE_CHANGED window event reverse channel; dashbar centre swaps Pause↔Play in lockstep with real game state).
