...
## Status: R82 open — R82.13 continuous-improvement: integration polish — info-block settles in sync with preview slide. Closes a visual fragmentation issue: when the user navigated prev/next, the preview image slid via its sibling `AnimatePresence`, but the category badge + description below it pop-changed instantly. Eye tracked the slide up top, then the info popped below — felt disjointed. Fix wraps the info block as a `motion.div` keyed on `selectedGame`, mounting with `initial={{ opacity: 0, y: 6 }} → animate={{ opacity: 1, y: 0 }}` over `slideTransition` (0.26s `cubic-bezier(0.25, 0.1, 0.25, 1)` — reuses the existing ease var, so timing is identical to the preview slide by construction). Y-lift (not X-slide) is deliberate — horizontal motion would compete with the preview's X-axis slide, making the panel feel like two sliding doors out of phase; a subtle vertical settle reads as "the info follows into place" without crosstalk. No exit animation (the whole block remounts on key change, old DOM just detaches); avoids `AnimatePresence` overhead for a one-direction transition. Reduced-motion branch passes `initial={false}` to skip the enter animation entirely — Framer Motion shorthand for "no starting state, mount in final position". Zero state changes, zero new CSS, single new `useMemo` for the transition variants. Gates: lint 0 errors / 10 intentional warnings, tsc clean, 23/23 unit tests (App + GamePortal), build clean (7.44s). Ralph does NOT write terminator — Tom's manual call post-review.
