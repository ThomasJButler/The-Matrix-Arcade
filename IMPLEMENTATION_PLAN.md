...
## Status: R82 open — R82.13 continuous-improvement: visual detailing — title typography (layered phosphor glow + CRT chromatic aberration). Inline `style={{ textShadow: ... }}` on the ASCII-title `<pre>` lifted into a dedicated `.ipod-title-glyph` CSS class so the shadow stack can be tuned without touching `GamePortal.tsx`. New stack reads as: (1) tight `1px white-green` core (the lit phosphor centre), (2) `6px / 16px / 30px` cascading green halo (preserves the existing CRT-bloom feel), (3) `±0.5px` cyan/magenta pair that mimics shadow-mask colour-fringing on a real CRT — half a subpixel each side, so it reads as authentic aberration not a stylised filter, (4) `0 1px 2px black` drop for legibility against the gradient overlay. **Hard-zeroed `letter-spacing: 0`** because the titles are block-character ASCII art (`<pre>` from `lib/asciiArt.ts`) — the previous inherited `0.05em` from `.ipod-title-overlay` was pushing every column out of vertical alignment, so the art was subtly skewing wider per row. Also locked `font-feature-settings: "kern" 0` (kerning is meaningless for ASCII columns and was a silent perf cost), `text-rendering: geometricPrecision` and `-webkit-font-smoothing: antialiased` for crisp 6–9px glyph rendering. Removed `letter-spacing: 0.05em` from `.ipod-title-overlay` (it was only ever inherited by the `<pre>`; the `sr-only h2` doesn't need it). Gates: lint 0 errors / 10 intentional warnings, 23/23 unit tests (GamePortal + App), build clean (7.41s). Ralph does NOT write terminator — Tom's manual call post-review.
