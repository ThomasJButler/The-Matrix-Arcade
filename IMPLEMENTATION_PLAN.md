...
## Status: R82 open — R82.13 continuous-improvement: category badge polish. Extracted the portal category pill from inline-style + Tailwind soup into a dedicated `.ipod-category-badge` CSS class in `animations.css`. Upgrades the badge to authentic iPod chrome: beveled pill edges (lighter top border `rgba(0,255,0,0.38)` + darker bottom `rgba(0,0,0,0.5)` for a "pressed inset" depth that Tailwind's single-colour borders can't express), vertical gradient fill (subtle green top → darker bottom for volumetric feel), dual-layer text-shadow (green glow + 1px black drop for debossed legibility), layered box-shadow (inner groove highlights + subtle outer green halo + 1px black drop shadow). Tightened typography: 9px Press Start 2P, 0.24em tracking. No inline styles remaining — cleaner JSX, animations/design details all in CSS. No new animations introduced so reduced-motion cohort unaffected. `mb-1.5` spacing preserved for layout parity. Gates green (tsc clean, lint 0 errors, App.test.tsx 19/19, build 7.50s clean). Ralph does NOT write terminator — Tom's manual call post-review.
