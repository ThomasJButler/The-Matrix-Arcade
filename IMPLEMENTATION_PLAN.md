...
## Status: R82 open — Playtest refinements R82.14–R82.20 added 2026-04-18 (height reduction, button swap, close-button removal, in-play width expansion + dashbar transformation) — ship BEFORE R82.13 continuous-improvement resumes
...
- [ ] **R82.14 — [P1]** Static portal height reduction. Portal currently occupies ~95% of viewport height at 1280×800 — target ~60-70%. Reduce clickwheel diameter (~180px → ~140px), tighten vertical gaps between screen / wheel / footer, consider 2-line description ellipsis. Preserve 16:9 screen aspect. **Files**: `src/components/GamePortal.tsx`, relevant CSS.
- [ ] **R82.15 — [P1]** Clickwheel button swap: PLAY → centre, SCORES → bottom. Currently centre = ● (scores, small dot), bottom = ▶/❚❚ (play). Swap so centre becomes the visually dominant play button (primary CTA) and bottom becomes scores (smaller). Update keyboard mapping: `ENTER`/`SPACE` = centre = play, `↓` = bottom = scores. **Update keyboard hint footer text** from `↓ PLAY • ENTER SCORES` → `↓ SCORES • ENTER PLAY`. **Files**: `src/components/GamePortal.tsx` (clickwheel JSX + keyboard handler + footer text).
- [ ] **R82.16 — [P1]** Remove the X close button at `GamePortal.tsx:254-261` (only renders during play). Exit-to-grid function stays bound to ESC key. Additionally: wire the MENU clickwheel position to act as "back/exit" when `isPlaying === true` (dual function: HOW TO PLAY modal when browsing, exit-to-grid when playing). Keep mute indicator (VolumeX icon) — that's informational, not an action.
- [ ] **R82.17 — [P1]** In-play portal width expansion. When `isPlaying === true`, animate portal width from static (~600-700px) to ~90-95% viewport width (capped at 1600px for ultrawide). Smooth transition 250-400ms spring easing via Framer Motion `<motion.div layout>`. When game exits, animate back to static width. **Tom's blocker**: CTRL-S currently unplayable due to cramped width — fixing this unblocks playtesting.
- [ ] **R82.18 — [P1]** Clickwheel → dashbar transformation (the big one). When `isPlaying === true`, morph the circular clickwheel into a horizontal slim bar (~50-60px tall) along the bottom of the widened portal. Dashbar layout: `[MENU] [◄◄] [▶/❚❚] [►►] [● scores] [🔇 mute]`. Framer Motion layout animation, ~300-500ms ease-in-out. When exiting play, morphs back to circular wheel. Likely extract `<ControlSurface>` sub-component that renders either mode based on `isPlaying`. **Complexity**: may take 2-3 iterations — first basic conditional layout, then smooth animation, then polish.
- [ ] **R82.19 — [P2]** Visual regression baselines for new layout. Regen both static (post R82.14 + R82.15) and in-play (post R82.17 + R82.18) baselines, darwin + linux (Docker). Commit separately: `R82.N-visual: baseline update — static height + in-play dashbar`.
- [ ] **R82.20 — [P1]** Verification + Discovered Work flush. Gates: lint + build + test + e2e + visual. **Critical playtest**: launch Snake Classic → play → exit (verify dashbar morph + width expansion + no jank). Launch CTRL-S → verify it's now playable in the widened portal (Tom's primary acceptance criterion). Log findings under `### R82 Discovered Work`. **Do NOT write R82 terminator** — R82.13 continuous-improvement continues per continuous-improvement pattern.
