...
- **Last Updated**: 13 April 2026 (R45 -- Accessibility audit + fixes)
...
- **R45 Accessibility audit and fixes**: Fixed WCAG 2.1 AA critical and high-priority issues. Text contrast: raised muted-green opacity in LandingPage from /30-/55 to /60-/70 range (description, inspiration, footer, subtitle). Added skip-to-content link to App.tsx. Added aria-live mute announcement. Added aria-expanded/aria-controls to keyboard controls toggle and Games nav button. Added aria-pressed to category filter buttons. Made game card images decorative (alt="" with aria-hidden). PhaserGame.tsx: added role="application" and aria-label, fixed click-to-play overlay with tabIndex=0 and keyboard handler, replaced outline:none with outline:transparent. Added prefers-reduced-motion media query disabling phosphor-glow, pulse, CRT scanlines, and transition animations.
