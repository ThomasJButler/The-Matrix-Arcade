...
## Status: R82 open — R82.13 continuous-improvement: clickwheel animation polish — keyboard press-flash. Closes a subtle UX gap exposed by our roving-tabindex pattern: `tabIndex={0}` on the toolbar + `tabIndex={-1}` on buttons means arrow keys fire `mapping.action()` on the parent handler, so individual zones never see `:active` and keyboard users miss the tactile depression mouse users get. Added `.is-pressed` class toggled for 200ms after each wheel-action keypress via a new `triggerPressFlash(index)` callback — mirrors the `triggerRotation` state/timer/rAF pattern exactly (null → rAF → index → setTimeout clear) so back-to-back presses of the same key re-trigger the animation (browsers coalesce class changes in the same paint frame, so the null-intermediate is load-bearing). Two CSS keyframes added in `src/styles/animations.css`: `clickwheel-zone-press` (scale 1 → 0.95 → 1, bg fade 0.05 → 0.18 → 0.05 at 180ms `cubic-bezier(0.34, 1.56, 0.64, 1)` — the same overshoot curve already used on hover/press transitions) and `clickwheel-centre-press` (preserves `translate(-50%, -50%)` while scaling 1 → 0.92 → 1, with a brief deeper inset shadow). Both classes appended to the existing `prefers-reduced-motion: reduce` `animation: none !important` block so keyboard users with motion sensitivity get instant, non-animated feedback. Scoped to wheel-only (not dashbar) since in-play keyboard only handles Escape. Cleanup `useEffect` now clears both `rotationTimer` and `pressTimer` on unmount to prevent stale-state bugs. No JSX structure change — each of the 5 wheel buttons just conditionally appends `' is-pressed'` based on `pressedIndex`. Gates: lint 0 errors / 10 intentional warnings, tsc clean, 23/23 unit tests (App + GamePortal), build clean (8.40s). Ralph does NOT write terminator — Tom's manual call post-review.
