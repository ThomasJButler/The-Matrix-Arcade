...
## Status: **R85 open** (2026-04-20 — Tom's playtest notes on Matrix Invaders + Metris filed). R85 = **Invaders + Metris polish + 2 NEW globals** carved out of the originally-planned 8-game polish phase because Tom wants to test the remaining 6 games first before committing to their scope. Loop cap: **20 iterations**. **Stream G globals — both shipped 2026-04-21** (G1 high-score persistence + G2 dashbar trophy). **R85.I1 (enemy sprite redesign) shipped 2026-04-21**. **R85.I2 bullet-time HUD shipped 2026-04-21**. **R85.I3 bigger enemy bullets shipped 2026-04-21** — enemy bullet dimensions bumped from 3×6 → 8×18 (single source of truth via `GAME_CONFIG.ENEMY_BULLET_WIDTH/HEIGHT`, taller than `PLAYER_BULLET_HEIGHT=10` so incoming threats visually dominate player fire). Collapsed the pre-R85 dual-path ternary that sized bullets differently based on `_spriteMode` and `hasLaserRed` presence — every render path now agrees on the config value, and boss bullets scale up consistently to `ENEMY_BULLET_WIDTH+2 × ENEMY_BULLET_HEIGHT+4` to preserve the threat hierarchy. Procedural `bullet_enemy` texture rewritten as a layered threat: outer red halo (α=0.35) + solid red body (centre 60%) + 2px bright white hot-core column for contrast against the matrix-rain + scanline backdrop. New motion-trail system — `enemyBulletTrailAccum` throttles trail spawns to one per `ENEMY_BULLET_TRAIL_INTERVAL` (40ms = 25Hz) per active bullet, trails piggyback on the existing `particles` decay pool (no new lifecycle machinery), gated on `enemyBullets.length > 0` so idle scenes allocate nothing. Regression tripwire: 9 new tests covering size contract (width ≥ 8, height ≥ 16, height > player), spawn-dimension parity (`displayWidth/Height === config`), trail spawn cadence, sub-interval gate, idle no-spawn, trail placement behind bullet, and trail-particle decay pipeline. 2571/2573 unit tests green (2 pre-existing todo), tsc clean, lint clean, build green in 7.66s. Next: R85.I4 invisible shooter power-up bug. **R84 still awaiting sign-off** (R84.S7 `[DEFER]`, R84.CI bucket Tom-tick-only). R83.CTRLS umbrella also still pending Tom's CTRL-S end-to-end tick.
