...
## Status: R82 open — R82.13 continuous-improvement: integration polish for game-switch slide. Wired up the previously-stubbed carousel animation — `AnimatePresence mode="wait"` in `GamePortal.tsx` had a keyed `motion.div` but no `initial/animate/exit` props, so switching games snapped instantly; a leftover `.transition-enhanced` scale+rotate CSS animation was fighting framer-motion's transform ownership. Replaced with direction-aware slide variants (`slideDirection = ±1` threaded through `custom` prop on both `AnimatePresence` and `motion.div`): outgoing tile slides out in the same horizontal direction the incoming tile will arrive from, producing a seamless iPod-Classic film-strip pan (260ms, ease-out cubic `[0.25, 0.1, 0.25, 1]`, ~6px opacity blend). Title overlay moved INSIDE the motion.div so ASCII art rides with the image instead of popping in place. `shouldReduceMotion` branch collapses slide to pure opacity crossfade (no translate), preserving the reduced-motion cohort's no-vestibular-trigger guarantee. Cleaned up orphaned CSS: removed `@keyframes enhanced-transition` + `.transition-enhanced` class + its reduced-motion reference — was the only place in `src/` it was used. Gates green (tsc clean, lint 0 errors, App.test.tsx 19/19 incl. `transition-right` class assertion, build 7.40s clean). Ralph does NOT write terminator — Tom's manual call post-review.
