...
- [x] **R80.2 — [P1]** Phaser scaffold. Created `src/components/games/phaser/CtrlSWorld/` with 5 scenes: BootScene, MenuScene, ChapterHubScene (Citizen Sleeper-style grid with keyboard nav), NarrativeScene (placeholder for R80.3 typewriter engine), GameOverScene. Feature-flagged via `?phaser-ctrls` URL param in App.tsx — both React and Phaser versions coexist. 10 unit tests passing. Config defines custom scene keys (CTRLS_SCENE_KEYS) separate from standard SCENE_KEYS to avoid collisions.
- [x] **R80.3 — [P1]** Typewriter text engine. Built `TypewriterEngine` as a pure logic class in `engine/TypewriterEngine.ts` — state machine (IDLE→TYPING→WAITING→DONE), delta-based character reveal, skip-to-end on advance during typing, next-paragraph on advance during waiting. NarrativeScene wired with blinking cursor, completed paragraph stack (max 4 visible, older fade out), layout reflow. 24 engine unit tests + 10 scene tests = 34 total passing. No auto-advance — full user-controlled pacing per design spec.
- [x] **R80.4 — [P1]** Story data format + prologue port. Created `src/data/ctrlsChapters.ts` with typed `Chapter` schema (id, index, title, shortTitle, paragraphs, ascii header art, inlineAscii panels, puzzleTriggers). Ported all 14 prologue paragraphs verbatim, 2 inline ASCII panels (Protector AI + Silicon Valley bunker), 1 puzzle trigger (prologue_first_command after paragraph 4). NarrativeScene now loads chapter data via `getChapter()`, renders header ASCII with fade-in, shows inline ASCII panels between paragraphs, pauses engine for puzzle triggers and emits `openPuzzle` game event to React. ChapterHubScene uses shared `getChapterTitle()`. Added `MONO` font to MATRIX_FONTS for ASCII rendering. 23 unit tests for data integrity. All 57 CTRL-S tests pass.
- [x] **R80.5 — [P1]** Chapter 1 + Chapter 2 content port. Every paragraph, choice, branch, item reward preserved. Unit tests. ✅ Ported Ch1 (27 paragraphs, 2 puzzles, 4 characters) + Ch2 (25 paragraphs, 6 puzzles) verbatim from old React. 58 tests pass.
- [x] **R80.6 — [P1]** Chapter 3 + Chapter 4 content port. Ported Ch3 (23 paragraphs, 3 puzzle triggers) + Ch4 (47 paragraphs, 6 puzzle triggers) verbatim. Converted Ch4 emoji ASCII header to pure ASCII. UK English normalisation (prioritised, harmonising). All 92 CTRL-S tests pass.
- [x] **R80.7 — [P1]** Chapter 5 content port + cross-chapter continuity check. Ported Ch5 (26 paragraphs, 1 puzzle trigger ch5_final_wisdom@15, ASCII sun-ray header). UK English normalisation (marvelled, utilising, centre). Added tests for ch3/ch4/ch5 content integrity + 12 cross-chapter continuity tests (sequential indices, character presence across chapters, puzzle trigger bounds, paragraph non-emptiness). Note: 2 orphaned puzzles (ch5_async_await, ch5_git_riddle) exist in puzzles.ts with item rewards but were never wired in original — preserved for Tom's R81 review. 110 tests pass.
- [x] **R80.8 — [P2]** Choice UI system. Added `ChoiceTrigger`/`ChoiceOption` interfaces + `getChoiceTriggerForParagraph()` helper to data layer. NarrativeScene renders choices as Container+Graphics buttons with: keyboard navigation (↑↓), ENTER/click to confirm, hover/select highlight states (border glow + text colour shift), particle burst on selection (12 radial particles), smooth fade in/out transitions (400ms), `waitingForChoice` gate for engine+input. Choice events emitted to React via `pause` event with `action: 'choice'`. E2E test state extended with `waitingForChoice`, `activeChoices`, `selectedChoiceIndex`. No actual story choices added yet — system ready for R81 content. 1,140 tests pass.
- [x] **R80.9 — [P1]** Asset sourcing from dump. Catalogued all 4 major dump folders (Hologram Interface, CyberPunk, Character sprites, Audio). Sourced and deployed 48 assets total: 10 UI elements (panels, buttons, slots), 11 icons, 4 backgrounds (cyberpunk city + hub nodes, resized to 800×600 via sips), 4 protagonist sprites (Cyberpunk character), 11 SFX (Matrix trilogy kit → MP3 via ffmpeg), 8 music tracks (WAV→MP3 at 192kbps). All deployed to `public/assets/ctrl-s/{ui,icons,backgrounds,portraits,audio/sfx,audio/music}`. BLOCKED-ART-NEEDED: 6 NPC portraits (Mana Seed requires multi-layer compositing tool), 5 backgrounds (no suitable source in dump). Updated ASSETS_NEEDED.md with full source notes.
- [x] **R80.10 — [P2]** Character portrait panel system. Added `CharacterDef` registry (7 characters with unique colours) and `PORTRAIT_CONFIG` to config.ts. Extended `Chapter` interface with `speakers?: Record<number, string>` parallel metadata — all 6 chapters annotated with speaker assignments based on content map. BootScene preloads 4 protagonist portrait variants. NarrativeScene renders portrait panel (70×70 bordered frame with portrait image or styled monogram fallback for NPCs without art), character name label, fade transitions on speaker changes (300ms), automatic text indentation when portrait visible. Portrait Y syncs with body text layout. 144 tests pass (17 config + 127 data incl. speaker lookup, data integrity, cross-chapter coverage). NPCs use coloured monograms (Señora=cyan, Elon=amber, Steve=orange, Billiam=purple, Samuel=pink, Protector=red) — deliberate terminal aesthetic, not placeholders.
- [x] **R80.11 — [P2]** Environmental backgrounds + parallax. 3-layer parallax system: far BG image (tinted, alpha 0.12, sine-drift), mid-layer themed particles (6 themes: binary/scanlines/datastreams/temporal/organic/rising-light), near-layer matrix rain. 3 new backgrounds sourced from dump (digital-construct from frame5, dystopian-streets from frame10, new-dawn from PixelWhale red_cloud_noon). Per-chapter backgroundKey + backgroundTint + particleTheme in Chapter data. Hub scene uses hub-node background. All 41 CTRL-S tests pass.
- [x] **R80.12 — [P2]** PuzzleScene overlay. Wired NarrativeScene's `openPuzzle` game event through PhaserGame bridge (new `onGameEvent` + `gameRef` props). CtrlSWorld/index.tsx now stateful: intercepts puzzle events, looks up puzzle data via `getPuzzleById()`, renders `<PuzzleModal>` as React overlay on top of Phaser canvas, calls `resumeAfterPuzzle()` on NarrativeScene when puzzle closes. Reuses existing PuzzleModal with focus trap, timer, lifelines, SentientAI + CharacterConversation modals. 47 tests pass (6 new puzzle integration tests). Sound system wired for puzzle SFX.
- [x] **R80.13 — [P2]** InventoryScene overlay. Wired existing `InventoryPanel` React component as overlay on Phaser canvas, following the same bridge pattern as PuzzleModal. NarrativeScene I key → emits `openInventory` event → React mounts slide-out drawer with item grid (grouped by type: quest/consumable/collectible/special), descriptions, new-item badges, and use buttons. Added `waitingForInventory` flag to freeze typewriter engine while panel is open, with `resumeAfterInventory()` public method for React to call on close. Also wired item rewards into puzzle completion — `handlePuzzleComplete` now calls `getItemRewardsForPuzzle()` → `gameState.addItem()` via `GameStateContext`, so items are properly awarded when puzzles are solved. 47 CTRL-S tests + 23 InventoryPanel tests pass.
- [x] **R80.14 — [P2]** ChapterHubScene visual select. Full Citizen Sleeper-inspired redesign: 4-state tiles (locked/available/in-progress/complete) with per-status colours, puzzle progress bars, chapter badges, connection lines between nodes. Staggered Back.easeOut entrance animation (80ms per tile), select pulse tween, camera fadeOut launch transition. Registry-driven progress data ready for R80.15 save integration. Icons preloaded in BootScene. New `getChapterPuzzleCount()` data helper. HUB_CONFIG constants. 182 tests pass.
- [x] **R80.15 — [P1]** Save system integration. Bridged `GameStateContext` ↔ Phaser registry bi-directionally: React pushes `completedChapters`, `completedPuzzles`, `currentChapter` into registry via `useEffect` (scenes read live progress); Phaser scenes emit save-point events (`chapterComplete`, `chapterLaunch`, `choice`, puzzle complete) back to React which calls `gameState.completeChapter()`, `completePuzzle()`, `makeChoice()`, `setChapter()` + immediate `saveGame()`. Added `CTRLS_REGISTRY_KEYS` constants to config.ts, replacing raw strings in ChapterHubScene. Fixed locked-chapter bug (chapters beyond currentChapter now correctly show as 'locked' instead of 'available'). `gameData.stats` undefined crash was already fixed in R78.11 with optional chaining — no new migration needed, save format unchanged. 56 CTRL-S tests pass (5 new: registry keys, chapter status, inventory resume).
- [x] **R80.16 — [P2]** Shatner TTS integration. Added `onParagraphStart` callback to TypewriterEngine (fires on start, startFromParagraph, nextParagraph). NarrativeScene emits `voiceStart` (with paragraph text) and `voiceStop` (on user skip or scene shutdown) game events through the existing React bridge. CtrlSWorld/index.tsx wires `useShatnerVoice` hook — speak on voiceStart, stop on voiceStop/mute. ShatnerVoiceControls panel overlaid in top-right corner with toggle, settings, and test phrases. Respects isMuted prop. 4 new TypewriterEngine tests, 168 voice+CTRL-S tests pass.
- [x] **R80.17 — [P2]** Ambient music bed. Added `musicTrack` field to Chapter interface and assigned all 6 chapters unique tracks from R80.9 sourcing (prologue-brothers, ch1-moonlight, ch2-cyberpsychotic, ch3-resonance, ch4-epic, ch5-cyberpunkin). Centralised `MUSIC_TRACKS` constant in config.ts (8 tracks: menu, 6 chapters, credits). NarrativeScene calls `playBackgroundMusic()` on create and `stopBackgroundMusic()` on shutdown. MenuScene + ChapterHubScene play menu theme. Track transitions are automatic via the existing HTML Audio bridge (same-track no-ops, new-track replaces). Volume control via existing useSoundSystem master/music sliders. 4 new music tests (track count, per-chapter assignment, uniqueness, path validation), 64 CTRL-S tests pass.
- [x] **R80.18 — [P2]** Character SFX. Added 4 procedural typewriter tick variants to SOUND_LIBRARY (protagonist=high sine 800Hz, antagonist=low sawtooth 180Hz, npc=mid triangle 440Hz, narrator=soft sine 550Hz). `CHARACTER_TICK_MAP` in config maps each character to archetype tick. `onCharRevealed` callback fires per character, throttled to every 3rd char to prevent audio spam. Added 8 CTRL-S stinger SFX to AUDIO_FILE_MAP (puzzle-appear, puzzle-solved, puzzle-failed, chapter-start, chapter-complete, dramatic-sting, reveal, transition) from R80.9 sourced MP3s. Upgraded NarrativeScene: chapter-start stinger on create, chapter-complete stinger on finish, puzzle-appear stinger on trigger, reveal stinger on choice. Puzzle solve/fail stingers wired in React bridge. `STINGER_KEYS` + `NARRATOR_TICK` constants in config.ts. 5 new tests (tick mapping, stinger keys), 69 CTRL-S tests pass.
- [x] **R80.19 — [P2]** ASCII title + MenuScene polish. Rewrote CtrlSMenuScene from scratch (now extends BaseScene directly instead of MenuScene). Large box-drawing ASCII title ("CTRL-S") with staggered cascade entrance (60ms per line, Back.easeOut), title glow pulse (Sine.easeInOut breathing), "JACK IN" start button, camera fadeOut transition to chapter hub, transition stinger SFX. Fixed the "too small" bug — title was plain 32px text, now full-width ASCII art at 12px mono. Citizen Sleeper atmosphere: 25-drop matrix rain, subdued controls text (30% alpha), stats line at 60% alpha, staggered reveal sequence (title → subtitle → tagline → stats → button → controls). 69 CTRL-S tests pass.
