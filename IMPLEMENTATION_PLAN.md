...
## Status: R82 open — Round 2 playtest refinements: R82.21–R82.28 [x]. R82.29 (manual human playtest) still unchecked — Tom to action. R82.13 continuous-improvement resumed: preview-image idle prefetch — 12 carousel preview URLs now warm the browser image cache in the background so arrow-keying to a previously-unvisited game never shows a cold-fetch flash. Previously the `<img>` tag remounted with a new `key={selectedGame}` on every carousel change (AnimatePresence pattern), and the 0.26s slide transition could finish before a cold Cloudinary request returned — leaving a broken-image square mid-slide for the four legacy games (`ctrlsthegame_m1tg5l`, `matrixsnake2_jw29w1`, `vortexpong2_hkjn4k`, `matrixcloud_rw8hsa`) whose previews are external URLs rather than Vite-bundled imports. Fix: one `useEffect([games])` schedules `prefetch()` via `requestIdleCallback(..., { timeout: 2500 })` after mount; `prefetch()` iterates `games` and calls `new Image(); img.decoding = 'async'; img.src = preview` for each entry. `new Image().src = …` hits the same browser image cache that `<img>` tags read from — no DOM churn, no layout cost, no new React element. `requestIdleCallback` yields to the initial paint + the current game's own image load, so the prefetch never competes with first-frame responsiveness; Safari < 16.4 fallback is a plain `setTimeout(prefetch, 1500)`. Cleanup cancels the handle if the component unmounts or `games` reidentifies before prefetch fires. Effect of this polish: once the browser has been idle ~2.5s after mount, arrow-keying through the 12 games is zero-network — every preview loads synchronously from cache, the AnimatePresence slide completes against a fully-painted image, and the "Cloudinary flash" on un-visited games is gone. Since local bundle imports (`matrixInvadersPreview` etc.) are already in memory, the prefetch is effectively a no-op for those eight — the network win is concentrated on the four cloudinary URLs. Why prefetch-all rather than neighbours-only (N-1, N+1): 12 images total, each a tiny thumbnail (~20-40 KB), so the total payload is < 0.5 MB and fires once per mount; a neighbours-only approach would add bookkeeping (track visited set, dedupe across direction changes) for minimal savings. Local `IdleAPI` type widening (inline intersection) keeps the narrowing scoped rather than polluting global ambient declarations — `requestIdleCallback` isn't in TypeScript's built-in lib.dom under the project's strict target. **Gates all green**: lint 0 errors / 11 pre-existing warnings, tsc clean, 58/58 App + GamePortal + KeyPanel tests pass (no test churn — passive perf improvement, no new behaviour to assert), build clean (8.28s). Previous iteration (clickwheel hover-glow radial LED ring): still deployed; this iteration targets carousel perception quality rather than interaction feel.
