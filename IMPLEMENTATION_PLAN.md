...
- [ ] **R83.G4 [P1]** MenuScene start-button position — across Snake, Matrix Bird, Vortex Pong, CTRL-S the "Start / JACK IN / Press ENTER" button sits too high and overlaps with chapter text, mission select, or the pause bar. Audit all 12 MenuScenes, bump the button down by a uniform constant, preferably hoisted into BaseScene as `MENU_START_BUTTON_Y_OFFSET` so all games inherit. Tom (CTRL-S note): *"move the Jack in buttin down a bit . chapters are overlapping the mision Select And the start from beginning button is overlapping with the pause button"*.
