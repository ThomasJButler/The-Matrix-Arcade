...
- [ ] **R84.V2 [P1]** Matrix Snake verification pass — same protocol, `MANUAL_TESTING_CHECKLIST_Snake_Classic.md`. Specifically verify: display label "Matrix Snake" everywhere, apple fits cleanly in grid, opening BGM not startling, power-up legend fades in/out, CRT atmosphere visibly denser than pre-S1, M key untoggles mute (Tom: *"can mute music but cannot unmute music in snake"*).
- [ ] **R84.V3 [P1]** Matrix Bird verification pass — same protocol, `MANUAL_TESTING_CHECKLIST_Matrix_Cloud.md`. Verify: "Matrix Bird" + bird image everywhere (Tom: *"just on the arcade layout with all games it says matrix bird"* — other surfaces may still show cloud), ground-death triggers Game Over, pause→resume has 5s countdown, birdFlap SFX is the tri-pluck, slow-powerup scales jump height, SFX volume dropped. iPod trophy shows Bird's score.
