...
## Status: R82 open — Round 2 playtest refinements: R82.21–R82.28 [x]. R82.29 (manual human playtest) still unchecked — Tom to action. R82.13 continuous-improvement resumed: clickwheel rotation feedback extended to jump-navigation keys — the wheel now physically spins in the direction of travel when the user presses Home, End, or any number key 1–9, matching the muscle-memory pattern already established for the arrow keys (ArrowRight → spin right → next, ArrowLeft → spin left → prev). Previously these jump-nav keys silently swapped the carousel slide with no wheel-level feedback, which broke the iPod physical metaphor: a tap on `7` from index `2` should feel like forward spin, not a teleport. Implementation: a single local `jumpTo(idx)` helper captures the preventDefault + playClick + directional `triggerRotation` call (`idx > selectedGame` → `'right'`, `idx < selectedGame` → `'left'`, equality → no-op) + `onJumpToGame(idx)` delegation. The helper lives inside the existing `handleWheelKeyDown` `useCallback` so the branch structure stays flat and dedup'd: Home becomes `jumpTo(0)`, End becomes `jumpTo(games.length - 1)`, and the `'1'`…`'9'` branch becomes `jumpTo(parseInt(e.key, 10) - 1)`. **Why reuse `triggerRotation` rather than a bigger-rotation "fast-spin" variant**: the fixed ±3° animation is already the rotational language users have learned from R82.7; introducing a distance-scaled rotation (e.g. 6° for a 4-step jump, 9° for a Home-from-last) would double the animation vocabulary without a matching real-world iPod reference (real clickwheels detent the same amount regardless of scroll speed — acceleration is conveyed by ticks-per-second, not per-tick magnitude). The direction alone is the FEEL win; magnitude stays consistent. **Why gate on direction** (`idx === selectedGame` → no rotation): pressing `3` when already on index 2 is a no-op for state but would otherwise trigger a phantom spin, which feels broken on same-slot key-bounces. **Why keep audio unconditional**: `playClick()` fires even on same-slot jumps so the user hears acknowledgement of the input — consistent with every other clickwheel button's click SFX. **Dep array**: `selectedGame` added to the `useCallback` deps list (the new closure reads it at branch time). 5 new GamePortal tests lock in: forward jump adds `rotating-right`, backward jump adds `rotating-left`, same-slot jump adds no rotation class, Home/End direction correctness at both ends of the range, and `onJumpToGame` still fires with the correct index. **Subtle test-infra learning**: `triggerRotation` wraps `setWheelRotation(direction)` in a `requestAnimationFrame` (existing R82.7 pattern — clears the prior rotation then sets the new one on the next frame so the CSS animation re-triggers cleanly). jsdom's rAF is implemented as `setTimeout(fn, 16)` which defers past the synchronous `act()` block, so className assertions race the state flush. Fix: `beforeEach` stubs `window.requestAnimationFrame` to invoke its callback synchronously (`cb(0); return 0`), which flushes the state update within the same `act()` — pattern now available for future rotation/press-flash tests in the same describe. **Gates all green**: lint 0 errors / 11 pre-existing warnings, tsc clean, 30/30 GamePortal tests pass (5 new), 19/19 App tests pass, 9/9 KeyPanel tests pass, build clean (8.48s). Visual baselines untouched (rotation class presence is sub-second and conditional on key event — visual snapshots are taken at rest). Previous change (screen-level pause overlay): still deployed; this iteration adds keyboard-tactile parity.
