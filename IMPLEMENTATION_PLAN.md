...
## Status: **R85 open** (2026-04-20 — Tom's playtest notes on Matrix Invaders + Metris filed). R85 = **Invaders + Metris polish + 2 NEW globals** carved out of the originally-planned 8-game polish phase because Tom wants to test the remaining 6 games first before committing to their scope. Loop cap: **20 iterations**. **Stream G globals — both shipped 2026-04-21** (G1 high-score persistence + G2 dashbar trophy). **R85.I1 (enemy sprite redesign) shipped 2026-04-21**. **R85.I2 bullet-time HUD shipped 2026-04-21** — persistent `[B] BULLET TIME` label + magenta meter bar below the combo readout, gated activation (press B on empty meter silently no-ops, must refill to full), 5s visible drain during active, 10s refill during cooldown, `BULLET TIME READY` pulse fires once per rising-edge (edge-latched via `bulletTimeWasReady` so it never re-fires while held at full). New `BULLET_TIME_COOLDOWN: 10000` in config balances the verb — rare enough to feel earned, fast enough that players re-learn the ability mid-wave. Regression tripwire: 9 new tests covering initial-state armed latch, gate-below-full, latch-arm-on-activation, drain proportional to duration, drain floor-at-0, refill proportional to cooldown, refill-ceiling + single-edge pulse, no-pulse when already ready, plus `bulletTimeCharge` added to the `getTestState` contract (2562/2564 unit tests green, 2 pre-existing todo). Next: R85.I3 bigger enemy bullets. **R84 still awaiting sign-off** (R84.S7 `[DEFER]`, R84.CI bucket Tom-tick-only). R83.CTRLS umbrella also still pending Tom's CTRL-S end-to-end tick.
