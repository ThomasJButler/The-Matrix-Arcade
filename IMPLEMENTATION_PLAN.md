...
- [x] **R87.C3 [P1]** **Braking / stop mechanic `[x]`** — Tom's 2026-04-22 playtest *"The player should be able to stop. This is to avoid hitting things by accident and provide more control"* + *"The player also needs a bit more control over their speed and jumping height"* shipped 2026-04-24. Root cause audit surfaced an unexpected finding: CloudJumper pre-R87.C3 had **no horizontal movement at all** — the player was pinned to `START_X=150` while the world auto-scrolled past, so there was no deceleration curve to tune, only a zero to replace. Fixed by adding a complete lateral-movement stack with drag-based release-to-stop: (1) **two new config dials** in `PLAYER` — `HORIZONTAL_SPEED=200` (px/s, ~2× SCROLL.SPEED_BASE=100 so the player can meaningfully outrun the world but under SCROLL.SPEED_MAX=300 so lateral alone can't beat the camera and break the side-scroller feel) and `HORIZONTAL_DRAG=800` (px/s² Phaser arcade drag on the X axis; full-speed → 0 decay in 0.25 s, which reads as "stop" when the key releases without feeling robotic); (2) **`createPlayer()` updated** — `body.setDrag(0, 0)` → `body.setDrag(PLAYER.HORIZONTAL_DRAG, 0)` so X drag decelerates lateral velocity automatically when no move key is held, Y drag stays 0 so falls remain gravity-driven unchanged; (3) **four new key fields** on `CloudJumperGameScene` — `moveLeftKey` + `moveLeftKeyA` + `moveRightKey` + `moveRightKeyD`, bound in `setupInput()` inside the existing `waitForKeyboard` callback alongside the jump keys so setup ordering is preserved, typed `Phaser.Input.Keyboard.Key | undefined` because `waitForKeyboard` is async and test setup can bypass it; (4) **new `handleHorizontalMovement()`** wired into `update()` between `updateObstacles` and `checkGameOver` (BEFORE `enforceCeiling` so boundary clamp wins in edge cases). Control model: LEFT/A drives `setVelocityX(-HORIZONTAL_SPEED)`, RIGHT/D drives `setVelocityX(+HORIZONTAL_SPEED)`, no key held leaves it to Phaser drag to decelerate — the "stop" beat Tom asked for is the drag-decay, not an explicit `setVelocityX(0)` call. Both-keys-held resolves to no-op (drag kicks in) so a player mashing both directions can't accumulate invisible velocity. `isCountingDown` + `isGameOver` short-circuit the handler so late input can't perturb the pre-game countdown window or the death tween. Boundary clamp pins `player.x` to `[WIDTH/2, CANVAS_WIDTH - WIDTH/2]` with into-wall velocity zeroed (away-from-wall velocity preserved so leaning against a wall while pressing the opposite direction releases cleanly the moment the wall clears). MenuScene controls legend updated with a new row `LEFT/A · RIGHT/D: Move | Release: Stop` at ratio 0.59, existing rows shifted up 0.04 to keep the block clear of the START button band. `exposeTestState` extended with `playerX` + `velocityX` for E2E watchers. **+28 regression tests** in a new `R87.C3 — Lateral movement + release-to-stop` describe block at the end of `GameScene.test.ts`, split across six micro-blocks: (1) **Config dials + ratchets** ×6 — HORIZONTAL_SPEED===200, HORIZONTAL_DRAG===800, both positive finite, SPEED<SCROLL_MAX (side-scroller feel lock), SPEED>SCROLL_BASE (lateral agency lock), DRAG≥4×SPEED (sub-second stop feel-dial ratchet — a future rebalance that tweaks both in lockstep stays green; a unilateral drag-halving fails and surfaces the regression before Tom's next playtest). (2) **Key-held velocity writes** ×5 — LEFT, A, RIGHT, D all produce correct signed velocity; LEFT+A both held produces single write (no doubled magnitude). (3) **Release-to-stop** ×2 — no key held produces zero setVelocityX calls (drag runs un-overridden), both LEFT+RIGHT held produces zero calls (conflict resolves to drag, not arbitrary pick). (4) **Scene-state guards** ×4 — isCountingDown short-circuits, isGameOver short-circuits, missing body no NPE, missing player no NPE. (5) **Canvas boundary clamp** ×6 — player.x<halfWidth pulled to halfWidth, player.x>maxX pulled to maxX, into-wall -vx zeroed at left, into-wall +vx zeroed at right, away-from-wall +vx at left NOT zeroed (preserves lean-away recovery), valid mid-canvas x no-op. (6) **Static source tripwires** ×5 — handleHorizontalMovement defined private, update() wires the call, createPlayer uses PLAYER.HORIZONTAL_DRAG (not the legacy `0`), setupInput binds LEFT/RIGHT/A/D, handler body contains isCountingDown + isGameOver guards. Test harness: `createTestScene` player body mock extended with `setVelocityX: vi.fn()` so tests can assert write calls; `stubMoveKeys(scene, opts)` helper scoped inside the C3 block sets `isDown` flags on the four key fields per-test for clean polling exercises. Gates: tsc clean, lint 0 err (4 pre-existing warnings), **3373 tests pass + 2 todo** (+28 from R87.C2's 3345), 61 test files, `npm run build` clean 7.73s. Tom-tick awaited on next CloudJumper hand-playtest — LEFT/A should now slide the player left at 200 px/s, RIGHT/D should slide right, releasing the key should decelerate to a clean stop inside 0.25 s, lateral input during the 5-second countdown should be ignored, and pushing against either canvas edge should stop cleanly without rubber-banding.
