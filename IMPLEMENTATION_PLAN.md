...
## Status: R82 open — Round 2 playtest refinements: R82.21–R82.28 [x]. R82.29 (manual human playtest) still unchecked — Tom to action. R82.13 continuous-improvement resumed: dashbar keyboard navigation during play — the dashbar advertises `role="toolbar"` with `tabIndex={0}` on the parent and `tabIndex={-1}` on children (canonical roving-tabindex scaffold), but arrow keys were dead because `handleWheelKeyDown` early-returned on `isPlaying`. Only ESC worked. Keyboard users had no way to activate EXIT / PAUSE / MUTE via the toolbar they'd focused. Now ArrowLeft/Right rove focus between the enabled dashbar slots — EXIT (zoneRefs[0]), PAUSE-centre (zoneRefs[4]), and the conditional MUTE pill (new zoneRefs[5], only present when muted + handler supplied). SCORES (zoneRefs[2]) stays in the DOM but is `disabled` during play, so it's skipped in the rove list. Home/End jump to the first/last slot. Enter/Space on a focused slot fires its action (EXIT calls `onExit` via `captureSurfaceFocusIntent('wheel')`, PAUSE dispatches `PAUSE_REQUEST_EVENT`, MUTE calls `onToggleMute`) — but only when focus has actually landed on a slot, so a bare-toolbar Enter doesn't accidentally exit the game. Rove list built fresh each keypress so a mute-pill appearance/disappearance slots in/out naturally. Mute pill gains `tabIndex={-1}` + `zoneRefs[5]` slot to match the roving pattern (previously it was Tab-reachable — inconsistent with the rest of the toolbar). New `.dashbar-btn.is-pressed`, `.dashbar-centre.is-pressed`, and `.dashbar-mute-btn.is-pressed` CSS animations mirror the existing `clickwheel-zone-press` / `clickwheel-centre-press` pattern: 180ms spring curve (cubic-bezier(0.34, 1.56, 0.64, 1)), scale-down flash, theme-matched background tint (green for exit/pause, red for mute). Press-flash fires via existing `triggerPressFlash(index)` on every rove step + activation — keyboard users get the same visual depression feedback mouse users get from `:active`. Reduced-motion block extended with the 3 new `.is-pressed` compound selectors (belt-and-braces — `triggerPressFlash` already no-ops under `useReducedMotion`, but explicit `animation: none !important` keeps the cascade self-documenting). 7 new Vitest tests lock in the behaviour: ArrowRight rotation, ArrowLeft wrap, MUTE included-when-muted, Home/End extremes, Enter on focused EXIT, Space on focused PAUSE, Enter with no focused slot is no-op. **Gates all green**: lint 0 errors / 11 pre-existing warnings, tsc clean, 48/48 App + GamePortal + KeyPanel tests pass, build clean (8.28s). No visual baselines affected — focus state only renders on live keyboard interaction. Previous change (dashbar mute pill becomes interactive click-to-unmute button): still deployed, complementary to the new rove nav (mute pill is both Tab-addressable from mouse path and ArrowRight-reachable from keyboard rove).
