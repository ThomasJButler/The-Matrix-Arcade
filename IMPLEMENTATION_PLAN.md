...
## Status: **R86 open** (2026-04-22 late — R86.R3 shipped: MenuScene HOW-TO-PLAY / RESONANCE-card overlap fixed via nine exported layout constants + 15-test geometric contract). **Stream F all shipped + Tom-signed-off** (*"im happy with matrix frogger, its good"* 2026-04-22 — F6 ticked). **Stream G all shipped**. **Stream N N1-N4 shipped**; N5 **bumped P2→P1** per Tom's flow-broken playtest. **Stream A A1 shipped** — `useSaveSystem` refactored to a module-level singleton via `useSyncExternalStore`, fixes the cross-instance state-sharing gap that caused the Scoreboard modal to show stale data until a page refresh. A2 middle-box ghost-release + A3 coverage still open. **Stream R R1+R2 P0 blockers + R3 menu layout all shipped** (Rhythm Hacker playable and visually clean); R4 coverage refresh still open. Remaining R86 work: **N5 Neo Jump flow + A2/A3 Agent Chase + R4 Rhythm Hacker coverage + V1 baselines**. Loop cap: `./loop.sh 15`. R87 now covers final 2 games only (CloudJumper + CodeBreaker). Loop-sentinel rule preserved. **R85 awaiting sign-off** (Tom-tick only — phrase lives in R85 stub below). R84.S7 `[DEFER]` + R84.CI bucket + R83.CTRLS umbrella also Tom-tick only. Full R85 archive at [`COMPLETED_WORK.md § R85`](COMPLETED_WORK.md#r85--invaders--metris-polish--2-globals-2026-04-20--2026-04-21).
...
1. **Stream G — NEW Globals (R86.G1 → G3)**: blockers across multiple games, ship FIRST. G1 Neo-Jump-specific save path audit, G2 click-to-focus pattern (both games), G3 promote CTRL-S render config to shared Phaser baseline (affects all 8 `pixelArt: false` games). — **ALL SHIPPED**.
2. **Stream F — Matrix Frogger polish (R86.F1 → F7)**: F1-F7 all shipped; Tom signed off 2026-04-22 (*"im happy with matrix frogger, its good"*). — **ALL SHIPPED**.
3. **Stream N — Neo Jump polish (R86.N1 → N5)**: N1-N4 shipped; **N5 bumped P2→P1** 2026-04-22 based on Tom's flow-broken verdict (*"still too difficult... can't get momentum... 5s restart too heavy"*).
4. **Stream A — Agent Chase polish (R86.A1 → A3)** — **ADDED 2026-04-22** (lifted from R87 due to Tom's faster test cadence). A1 scoreboard integration (P0 — third game confirming save-path gap), A2 ghost-house release bug, A3 coverage. Parallel-safe with Stream N after G `[x]`.
5. **Stream R — Rhythm Hacker unblock (R86.R1 → R4)** — **ADDED 2026-04-22 late** because Tom couldn't even test the game in its current state (P-key conflicts with pause → every lane-4 press pauses mid-song; iPod dashbar chrome overlaps the lane hit-zone). R1 + R2 are **both P0** (game literally unplayable). R3 menu overlap is P1. R4 coverage. Parallel-safe with Stream A after G `[x]`.
6. **Stream V — Verification + baselines (R86.V1)**: ship LAST after all G/F/N/A/R `[x]`.
...
- [x] **R86.F6 [P2]** **Multi-level polish** — **Tom's 2026-04-22 sign-off: *"im happy with matrix frogger, its good"***. Finish-line works, multi-level feel lands cleanly. F6 closed without additional code changes — the 4 safety-net tripwire commits (`b9ee461` + `f720982` + `81684a1` + `f77a8c3`, +32 regression tests covering state persistence / timer isolation / difficulty-ramp arithmetic / playerDeath contract) lock the multi-level behaviour against future drift. Ticked post-playtest.
...
- [ ] **R86.N5 [P1]** **Neo Jump flow + difficulty second-pass** — Tom's 2026-04-22 playtest verdict post-N1: *"neo jump is still too difficult and there is too many bombs early in the game, meaning the player cannot get momentum and a feel for the game before they die and have to wait 5 seconds to start again. we can improve the flow of this game for sure"*. N1 helped but flow still broken on rapid retries. **Bumped P2 → P1**. Three concrete levers:
  - **(a) Further bomb density reduction in 0-300m** — drop spawn density another 25-30% on top of N1's numbers; reserve first 150m as "tutorial strip" with near-zero bombs so the player builds a basic mental model before threat density ramps. Likely needs `ENEMIES.SPAWN_ALTITUDE` bumped 800 → 1000 + `SPAWN_CHANCE_BASE` shaving 0.018 → 0.013. Preserve mid/late-game ramp (the N5 safety-net checkpoints at 1km/2km/5km/10km already pin the ramp shape — re-pin if bumping SPAWN_ALTITUDE shifts the boundary).
  - **(b) Post-death restart friction** — 5s countdown is too heavy when players die in <30s and want to retry fast. Option 1: shorten countdown to 2s on retry-within-30s (cold-start stays 5s so the first run feels deliberate). Option 2: skip countdown entirely on retry and show a paused "press any key to go" screen. Cross-game precedent: Pong/Snake/Bird all use the standard 5s BaseScene.startCountdown; a Neo-Jump-specific retry override is acceptable given the rapid-retry loop is this game's core loop.
  - **(c) Opening-beat spawn protection** — first 300-500ms post-spawn should feel empowering: guarantee no bomb within ~200px of player spawn X-column, brief invuln window on spawn/retry (300-500ms) with a visual "POWERING UP" indicator. Ensures the "bomb out of nowhere" deaths Tom flagged can't happen in the first second of any run.
  
  N5 safety-net tripwires (`f63a3b3`, +16 tests) already lock the difficulty ramp checkpoints — some may need re-pinning post-tuning to match new values. Don't regress N1's landing — N5 layers on top.

**Stream A — Agent Chase polish (added 2026-04-22, lifted from R87 per Tom's faster test cadence, ~3 iterations):**

- [x] **R86.A1 [P1]** **Scoreboard modal live-refresh gap — ROOT-CAUSED + FIXED** — diagnosis was subtler than the plan's original Zustand/`getState` guess. The codebase has NO Zustand store for save data and no one-shot `.getState()` reads; Scoreboard + AttractMode + GameHighScores all receive their props from App.tsx's `useSaveSystem()` hook. The bug was that **`useSaveSystem` is a `React.useState` hook, not a singleton** — every caller (App.tsx line 96, PhaserGame.tsx line 76, useAchievementManager, SaveLoadManager) got its OWN independent state bucket. Writes from PhaserGame's bucket landed in localStorage but never propagated in-memory to App's bucket, so the Scoreboard modal kept showing App's frozen snapshot until a page refresh re-read localStorage. This is exactly Tom's 2026-04-22 symptom on Neo Jump + Agent Chase: *"scores DO persist on refresh, but the Scoreboard modal is stale mid-session"*. **Fix**: converted `useSaveSystem` to a module-level singleton — `sharedSaveData` + `useSyncExternalStore(subscribeSaveData, getSaveDataSnapshot)` + `setSharedSaveData` funnel. Every hook instance now observes the same saveData reference, and any setter fires `saveDataSubscribers.forEach(fn => fn())` so every consumer re-renders synchronously. Exported `__resetSaveSystemForTest()` for vitest isolation (clears singleton + subscriber set between tests). The 9 in-hook `setSaveData(...)` call sites were migrated to `setSharedSaveData(...)` — this also resolved 9 new `react-hooks/exhaustive-deps` lint warnings by pointing callbacks at a module-stable reference. **Defensive Agent Chase `updateGameSave`** — shipped as promised belt-and-suspenders. Mirrors the Neo Jump R86.G1 pattern (getSaveData → merge prev stats → updateGameSave before gameOver), uses `longestSurvival` as the stats metric Agent Chase tracks (no combo). Already-shipped Neo Jump + Metris + AgentChase defensive writes stay as historical safety nets; the other 8 games (Rhythm Hacker, CodeBreaker, CloudJumper, Invaders, Snake, Pong, Bird, Frogger) are NOT retrofitted — the singleton fix obviates the defensive pattern everywhere at once, and adding 8 game-specific stats-merge blocks is scope creep better handled in R87 per-game polish sweeps if symptoms recur. **Regression coverage (+12 tests)**: (a) new file `src/hooks/useSaveSystem.cross-instance-sync.test.ts` (7 tests) locks the cross-instance contract — two concurrent `renderHook(() => useSaveSystem())` calls observe the SAME saveData reference, `addScore`/`updateGameSave`/`unlockAchievement`/`clearBoard` on one instance propagate to the other within the same render pass, unmount-one-instance doesn't break broadcasts to the other, `__resetSaveSystemForTest` drains subscriber list cleanly. (b) `AgentChase/scenes/GameScene.test.ts` gains 5 tests in a new `R86.A1 — defensive save-system persistence on final death` describe block — writes highScore+level+stats via updateGameSave, fires updateGameSave BEFORE gameOver (subscriber-sees-update-on-same-tick ordering lock), merges stats correctly (gamesPlayed+totalScore accumulate, longestSurvival keeps max), no-ops gracefully when registry is empty, defensive write skipped when lives remain (2→1 non-fatal hit). Gates: tsc clean, lint **0 err 4 warn** (down from 13 — 9 new `setSaveData` warnings resolved by migrating to module-stable `setSharedSaveData`), **2991 tests pass + 2 todo** (+12 from R86.R2's 2979), `npm run build` clean 8.22s. Tom-tick awaited: next playtest — finish ANY game with a new high score, open the Scoreboard modal WITHOUT refreshing, the new entry should appear immediately.
- [ ] **R86.A2 [P1]** **Middle-box agents not releasing** — Tom: *"some enemies are trapped in the middle box and don't come out of the box to chase the player, this makes the game much easier than it should be"*. Classic Pacman-genre bug — ghost-house release logic isn't firing for some agents. Audit `AgentChase/scenes/GameScene.ts` ghost-house release code: (a) is there a dot-count threshold gating each agent's release? (b) is the spawn-box exit-point pathfinding reaching all agents? (c) could staggered release timers be firing only the first 1-2 agents? Fix so all agents eventually exit and chase — but stagger release to preserve early-game breathing room (first agent immediate, second at 10 dots, third at 30, fourth at 60 — tunable). Don't over-correct into impossible difficulty; Agent Chase should still feel navigable.
- [ ] **R86.A3 [P1]** **Agent Chase unit-test coverage refresh** — cover A1 save-path round-trip + A2 ghost-release thresholds + static source check that every agent has a release-trigger (regression guard against future agents silently joining without release logic). Aim for +8-12 tests matching R86.N4/F7 cadence.

**Stream R — Rhythm Hacker unblock (added 2026-04-22, Tom cannot test current state, ~4 iterations):**

Tom's 2026-04-22 verdict: *"UNPLAYABLE - due to controls being overlapped by the play button, and we cannot use the P (QWOP) as this P key pauses the game. This needs to be fixed before I can continue with testing."* + menu-overlap screenshot. Both gameplay AND menu have blocking issues.

- [x] **R86.R1 [P0]** **P-key conflict between QWOP lane and global pause** — Rhythm Hacker uses QWOP for 4-lane split-hand hit keys; the arcade-wide `P = pause` binding set by `BaseScene._bindCommonKeys` intercepted every lane-4 press, pausing the song mid-note. Fix is a new `protected getPauseKeyCode(): number | null` virtual on `BaseScene` (defaults to `Phaser.Input.Keyboard.KeyCodes.P` so every other arcade game keeps the established muscle memory) consulted inside `_bindCommonKeys` — when `null`, the addKey call is skipped entirely. `RhythmHackerGameScene` overrides to return `null` with a full WHY block citing QWOP + the dashbar pause-button fallback. Pause stays reachable via the iPod dashbar (`PAUSE_REQUEST_EVENT` handler is independent of any keyboard binding). Diverged from the plan brief's "register `ESC` as sole pause toggle" suggestion because the brief's reasoning rested on a false premise (`BaseScene` *also* binds ESC → `handleExit`, not just the portal); inverting ESC to pause inside Rhythm Hacker would silently break ESC-exit consistency arcade-wide. MenuScene footer updated from `'ESC: Exit  P: Pause  M: Mute'` → `'ESC: Exit  M: Mute'` with an inline comment explaining the omission so a future cleanup pass doesn't "restore" the misleading hint. **+6 regression tests** in a new `R86.R1 — P-key pause override (QWOP lane 4)` describe block: (1) `getPauseKeyCode()` returns null on the override class (direct invariant — a refactor restoring the parent's binding immediately re-pauses on every lane-4 hit); (2) behavioural lock — calling `onKeyDown(3)` directly leaves `isPaused === false` and applies the empty-hit penalty (proves the lane-4 codepath ran instead of an early pause-toggle); (3) QWOP order locked at `LANES.KEYS === ['Q','W','O','P']` + `LANES.COUNT === 4` (anti-regression against any "fix" that rebinds lane 4 away from `P`); (4) static source check on `BaseScene.ts` confirming the default `getPauseKeyCode()` still returns `KeyCodes.P` (every other game keeps the binding); (5) static source check on `BaseScene._bindCommonKeys` confirming the `if (pauseCode !== null)` guard wraps the `addKey` call (a regression dropping the guard would re-bind P unconditionally); (6) static source check on `MenuScene.ts` asserting the footer no longer contains `P: Pause` (the menu hint must match real behaviour). Gates: tsc clean, lint 0 err (4 pre-existing warnings), **2979 tests pass + 2 todo** (+13 from R86.N4+ safety-net's 2966 — combined with R86.R2), `npm run build` clean 7.80s. Tom-tick awaited on next playtest — Rhythm Hacker should now be playable without lane-4 pauses.
- [x] **R86.R2 [P0]** **Lane hit-zone obscured by iPod dashbar chrome** — Tom's 2026-04-22 screenshot showed the dashbar (exit / help / pause / trophy icons) sitting visually on top of the bottom ~80 px of the Phaser canvas, with the pause button `II` directly on lane 2/3 hit-zones. Picked the plan's option B (in-canvas reposition) over option A (shrink HEIGHT) because option A would force an immediate parallel rework of the MenuScene HOW TO PLAY band — which the plan deliberately keeps as a separate task (`R86.R3`). With option B, canvas `HEIGHT` stays at 700 (avoiding the cascade through `BootScene` texture generation, MenuScene Y-positioning, and the GameOverScene), and the bottom 100 px becomes a safe zone the dashbar chrome can paint over without obscuring the playfield. Two paired dial moves in `config.ts`: `HIT_LINE_Y: 640 → 560` (lifts the hit line + key indicators 80 px above the dashbar footprint) and `SPAWN_HEIGHT: -50 → -130` (shifts spawn by the same 80 px so `(HIT_LINE_Y - SPAWN_HEIGHT) / SPEED * 1000` stays at exactly 1725 ms — note approach feel is preserved). Lane backgrounds stay anchored to `HEIGHT/2` so the highway still spans the full canvas; only the interactive band moves. Full WHY block in the `NOTES` config explains the dial dependency so a future tweak of either constant in isolation doesn't accidentally compress travel time and break the timing windows. **+7 regression tests** in a new `R86.R2 — Hit-zone clear of dashbar chrome` describe block: (1) `HIT_LINE_Y === 560` direct dial lock (a regression restoring 640 reproduces Tom's "unplayable" verdict); (2) `SPAWN_HEIGHT === -130` paired-dial lock; (3) derived travel-time invariant `(HIT_LINE_Y - SPAWN_HEIGHT)/SPEED*1000 === 1725 ms` (the gameplay-preservation tripwire — a single-dial move would change this); (4) key-indicator clearance: `HIT_LINE_Y + 35 ≤ HEIGHT - 100` (locks the dashbar safe-zone contract); (5) canvas dimensions unchanged at WIDTH=800 + HEIGHT=700 (anti-regression against an option-A drift that would silently break the menu); (6) hit-line texture's bottom edge `HIT_LINE_Y + 7 ≤ HEIGHT - 100` (every visible pixel of the bar clears the dashbar reserved area, not just the centreline); (7) miss-detection threshold `HIT_LINE_Y + 100 < HEIGHT` (locks that notes still get caught by the miss-detect window before falling off the canvas — a regression here would silently leak un-missed notes off-screen and break combo accounting). Gates: tsc clean, lint 0 err (4 pre-existing warnings), **2979 tests pass + 2 todo**, `npm run build` clean 7.80s. Visual baseline regen deferred to **R86.V1** (per the phase verification plan). Tom-tick awaited on next playtest — no chrome overlap on any lane, hit-zone fully visible above the dashbar footprint.
- [x] **R86.R3 [P1]** **Menu HOW TO PLAY band overlaps RESONANCE (5th difficulty card)** — root-caused to a layout that assumed 4 tracks: tracks walked downward from `y = 180` at `+90` spacing, and the HOW TO PLAY title was pinned to `HEIGHT - 180 = 520`. When `RESONANCE` was added as the 5th entry (`GAME_CONFIG.TRACKS[4]`), the last card's button-bg graphic landed at y=540 spanning y=510..570, while the 14 px HOW TO PLAY title (origin 0.5) rendering at y=520 actually spans y=513..527 — squarely on top of the card's top edge. The instruction lines at y=540 and y=555 landed inside the card body. Classic "anchored-to-bottom via `HEIGHT - offset`" bug that only bites once the mid-region content grows. Took the task brief's preferred fix path (shift HOW TO PLAY band down) but combined it with a 30 px upward shift of the track list so the cumulative spread stayed within the 700 px canvas without having to compress track cards. Also refactored all layout Y values out of the scene body into **nine named exported constants** (`TRACK_START_Y`, `TRACK_SPACING_Y`, `TRACK_BUTTON_HEIGHT`, `CLEARANCE_PX`, `HOW_TO_PLAY_TITLE_Y`, `HOW_TO_PLAY_LINE_1_Y`, `HOW_TO_PLAY_LINE_2_Y`, `CONTROLS_PROMPT_Y`, `CONTROLS_FOOTER_Y`) with a full WHY block at the top of `MenuScene.ts` — single source of truth means the next layout audit reads all nine values in one place. Final layout: TRACK_START_Y=150, TRACK_SPACING_Y=90 → last track centre 510, box bottom 540; HOW TO PLAY title at 560 (14 px → top 553 → **13 px clearance ≥ 10 px** contract); instruction lines at 580/595; controls prompt at 625; footer at 655 (40 px margin to canvas bottom). Diverged from the brief's fallback suggestion of "shrink card vertical padding by ~10px" because the 30 px upward shift of the track list alone gave enough room — reducing card padding would have made the 16 px track-name text feel cramped. Card hit-targets continue to track each track's `y` (the `Phaser.Geom.Rectangle(-250, -30, 500, 60)` hit area is relative to the container, so it moves with the card automatically; no manual adjustment needed). **+15 regression tests** in a new `MenuScene.test.ts` file with three describe blocks: (a) `Dial locks` (7 tests) pin each of the nine constants plus the 5-track count that the geometry maths depends on — a regression restoring `TRACK_START_Y = 180` re-reproduces Tom's overlap screenshot; (b) `Geometric invariants` (6 tests) lock the core contract `lastTrackBottomEdge() + CLEARANCE_PX ≤ HOW_TO_PLAY_TITLE_Y - TITLE_HALF_HEIGHT`, plus strictly-increasing row order (no reorder regressions), non-overlapping 10 px instruction bands, footer-inside-canvas, and `HOW_TO_PLAY_TITLE_Y + HOW_TO_PLAY_LINE_1_Y` both outside the 100 px dashbar reserved zone (R86.R2 contract carried forward); (c) `Source-code contract` (2 tests) read `MenuScene.ts` via `readFileSync`, strip block + line comments, and assert the stale `HEIGHT - 180/160/145/100/70` offsets appear nowhere in executable code — the WHY block can still reference them in prose for future-auditor context. The core `lastTrackBottomEdge + CLEARANCE_PX ≤ titleTop` test is the one Tom-facing tripwire: adding a 6th track without moving HOW_TO_PLAY_TITLE_Y breaks it immediately rather than silently re-overlapping inside Phaser. Gates: tsc clean, lint 0 err (4 pre-existing warnings), **3006 tests pass + 2 todo** (+15 from R86.A1's 2991 — exactly matches the new MenuScene.test.ts count), `npm run build` clean 12.96s. Visual baseline regen deferred to **R86.V1** per the phase verification plan. Tom-tick awaited on next playtest — HOW TO PLAY should sit cleanly below RESONANCE card with breathing room, all five difficulty cards fully visible + clickable.
- [ ] **R86.R4 [P1]** **Rhythm Hacker unit-test coverage refresh** — lock R1 P-key-gated-to-hitLane contract (P presses during scene active call `hitLane(3)` not `togglePause`), R2 hit-zone Y-position invariant + canvas-height-vs-dashbar-clearance anti-regression, R3 MenuScene layout ratios (HOW TO PLAY Y ≥ last difficulty card bottom + 10px). Also add a save-path round-trip test pre-emptively — given R86.A1 found Agent Chase has the same scoreboard gap, Rhythm Hacker likely does too (Tom couldn't test it to confirm). If R86.A1's grep audit flags Rhythm Hacker, fold that fix in here as R4 sub-commit. Aim for +10-12 tests.

**Stream V — Verification + Baselines (ship last, 1 iteration):**

- [ ] **R86.V1 [P1]** Visual baseline regen + full gate battery — after all G/F/N/A/R streams `[x]`. Commands: `npx tsc --noEmit` + `npm run lint` + `npm run build` + `npm test --run` + `npm run test:e2e` + `npm run test:visual -- --update-snapshots` (review diffs — Frogger finish-line fix + Neo Jump difficulty rebalance + Agent Chase ghost-release fix + Rhythm Hacker canvas resize may invalidate baselines). Commit baseline regens separately as `R86.V1-visual: baseline update — Frogger + Neo Jump + Agent Chase + Rhythm Hacker polish`. Verify WebGL warnings silenced across all 11 Phaser games post G3.
...
- R86.F1 – R86.F7 `[x]`
- R86.N1 – R86.N5 `[x]`
- R86.A1 – R86.A3 `[x]` (Agent Chase polish — lifted from R87 2026-04-22)
- R86.R1 – R86.R4 `[x]` (Rhythm Hacker unblock + polish — lifted from R87 2026-04-22 late; R1+R2 P0 blockers made game unplayable)
- R86.V1 `[x]`
...
- **2026-04-22 — R86.A1 cross-game audit: 8 games lack the defensive `saveSystem.updateGameSave` pattern** (Rhythm Hacker, CodeBreaker, CloudJumper, Matrix Invaders, Snake Classic, Vortex Pong, Matrix Bird, Matrix Frogger). Only Neo Jump (R86.G1), Metris (R85.G1), and Agent Chase (this iteration) have direct writes before their `gameOver()` call. **Decision: NOT retrofit all 8** in R86 because the `useSaveSystem` singleton fix (this iteration) eliminates the root cause — in-memory shared state now propagates writes across hook instances synchronously, so the defensive write is redundant belt-and-suspenders insurance rather than a bug fix. If R87's per-game playtest surfaces scoreboard symptoms, fold the defensive write into that game's polish iteration then. Retrofitting all 8 eagerly would be 8 × (stats slice design + 5 regression tests each) = ~40 commits of scope creep with zero functional benefit over the singleton fix.
...
- **2026-04-22 — R86.F6+ safety-net tripwires shipped ahead of Tom's F6 playtest** (logged as `F6 safety-net+` per the N2+ playbook). Ralph's reasoning: the original F6 safety-net (8 tests) locks state PERSISTENCE across level-up but leaves two real gaps. (a) **Timer isolation** — `armGameplay()` arms enemy + pill spawn timers ONCE with `loop:true`; `triggerLevelUp` must not touch `this.time` or those timers stop firing on Level 2+. That symptom matches Tom's F6 brief verbatim ("agents not respawning"). A future refactor calling `removeAllEvents()` on level-up to "flush stale callbacks" would silently break it, and Tom's playtest would be the first signal. (b) **LEVEL_5 boundary** — the `if (this.level >= 5)` gate at `triggerLevelUp` line 1293 has no test covering the operator choice; a refactor from `>= 5` to `=== 5` would silently break players reaching level 6+ via any future skip-level mechanic. +7 regression tests in a new `R86.F6+ safety-net — Level-up timer isolation + LEVEL_5 boundary` describe block: timer isolation ×2 (no `removeAllEvents`/`removeEvent`/`addEvent` calls during `triggerLevelUp`), LEVEL_5 boundary ×3 (level 4 doesn't unlock, level 5 exactly unlocks, level 8 still unlocks — locks `>=` not `===`), FIRST_CROSS idempotent unlock contract (fires on every cross, not just the first — guards against a "first" rename refactor), and updateUI ordering (`LEVEL: 3` written post-increment from level 2). Gates all green (2940 unit tests, tsc clean, lint 0 err, build 7.68s). No production code touched — pure coverage refresh.
- **2026-04-22 — R86.N4+ safety-net tripwires shipped ahead of Tom's N1/N2/N3 playtest** (logged as `F6+++ safety-net+` per the N2+/F6+/F6++/F6+++ cadence). Ralph's reasoning: Neo Jump's mirror of the F6+++ playbook. N4 locked `playerDeath`'s `shake(200, 0.012)` + `flash(120, 255, 0, 0, ..., 0.25)` literals and the existing Game Over block locks the red tint + `isGameOver` flag + "tween starts" + re-entry guard; G1 locks the `saveSystem.updateGameSave` branch. That left three invariant families unprotected on Neo Jump's death path: (a) **juice contract** — `playSound(SOUND_KEYS.GAME_OVER)` + the four-key tween motion signature (`targets=player, alpha=0, y=+100, angle=180, duration=500`) both unlocked; a refactor extracting a "death helper" could silently drop the sound or swap the rotation. (b) **onComplete ordering + gameOver payload** — chain must be `reportScore → saveSystem write → gameOver` with `highScore = max(score, highScore)` firing BEFORE reportScore so the scoreboard sees the new watermark; `gameOver`'s 6-arg payload has a fixed shape (score, reason `Altitude: Xm`, highScore, 2-row stats `[Enemies, Collectibles]`, level=`floor(alt/500)`, duration) — exact shape of the R85.G1 scoreboard regression, so payload drift here breaks both the modal layout and saved stats. (c) **playerSpriteMode branch** — sprite-mode path calls `updatePlayerTexture('death')`; programmatic-graphics path does not; a refactor that flips the default mode leaves the player dying while still looking alive. +9 regression tests in a new `R86.N4+ safety-net — playerDeath contract + onComplete chain (pre-Tom-tick)` describe block at end of `NeoJump/scenes/GameScene.test.ts` split into three micro-blocks: juice contract ×3 (playSound single-call + tween targets === player + 4-key motion literals), onComplete ordering + payload ×4 (invocationCallOrder reportScore-before-gameOver, highScore promoted pre-report on new-high, highScore preserved on low-score run, full 6-arg gameOver payload with `score=1500, reason='Altitude: 1500m', highScore=1500, [Enemies 5/Collectibles 12], level=3, duration=42000`), playerSpriteMode branch ×2 (updatePlayerTexture('death') when sprite-mode=true, NOT called when sprite-mode=false). Gates all green (2966 unit tests, tsc clean, lint 0 err, build 7.63s). No production code touched — pure coverage refresh.
- **2026-04-22 — R86.F6++ safety-net tripwires shipped ahead of Tom's F6 playtest** (logged as `F6++` per the F6+/N2+/N5 cadence). Ralph's reasoning: the F6 + F6+ safety-nets lock state PERSISTENCE and TIMER ISOLATION across level-up, but still leave the *difficulty-ramp arithmetic* itself unlocked — specifically the two single-line magic numbers that shape how Level 2 feels different from Level 1. (a) **Per-level speed ramp** at `GameScene.ts:1068` — `const levelBonus = (this.level - 1) * 15` adds 15 speed units per completed level to `enemy.baseSpeed`. A refactor promoting the literal to a config constant (or dropping it entirely during cleanup) would flatten the ramp without tripping any runtime gate; Tom's F6 playtest would feel "Level 2 plays identical to Level 1". (b) **NEO-pickup level-gate** at line 1139 — `if (roll < 0.10 && this.level >= 2)` keeps NEO pills as a Level 2+ reward, honouring the tutorial-level anchor. A refactor dropping the `&& this.level >= 2` clause would introduce level-1 NEO drops (break tutorial curve); tightening it to `=== 2` would silently strand Level 5+ runs without NEO (misses the `>=` intent). (c) **First-wave density** — `spawnInitialEnemies` at line 1083 opens with `ENEMY_COUNT_BASE * 2` spawns; the `*2` multiplier has no test. +8 regression tests in a new `R86.F6++ safety-net — Difficulty-ramp arithmetic + NEO level-gate` describe block: (1) `spawnInitialEnemies` fires exactly `ENEMY_COUNT_BASE * 2` (= 6) times via `vi.fn()` stub of `spawnEnemy`; (2) `spawnPills` at level 1 with `Math.random=0.05` forced → `pillType !== 'neo'` + `pills.get` never called with `'neo_pickup'` (locks tutorial anchor); (3) `spawnPills` at level 2 with same roll → `pillType === 'neo'` + texture key `'neo_pickup'` (locks `>= 2` threshold); (4) `spawnPills` at level 7 with same roll → `pillType === 'neo'` (locks `>=` operator vs `===` regression); (5) static source check: `(this.level - 1) * 15` literal appears exactly once (locks line 1068 magic number); (6) static source check: `enemy.baseSpeed = ... difficultyBonus ... levelBonus` regex matches (locks both bonus terms in the arithmetic chain); (7) `SPEED_INCREASE_PER_100 === 10` (distance-ramp multiplier — F6 state-persistence already locks maxDistance survives across levels, this anchors the weight); (8) `ENEMY_COUNT_BASE === 3` (first-wave anchor; the `*2` doubling test depends on this baseline). Harness addition: `beforeEach` seeds `Phaser.Math.Between` locally (the global Phaser mock in setup.ts leaves `Math` undefined) — mirrors the NeoJump test-file idiom so `spawnPills`'s col/row lookups don't crash. Gates all green (2948 unit tests, tsc clean, lint 0 err, build 7.62s). No production code touched — pure coverage refresh.
...
### R87 — Final 2 games: CloudJumper + CodeBreaker

Cadence update 2026-04-22 late: Rhythm Hacker also lifted into R86 Stream R (Tom couldn't test current state — P0 blockers). R87 is now the FINAL per-game polish batch covering the last 2 untested games. Contingent on Tom playtesting both + filling their `MANUAL_TESTING_CHECKLIST_*.md` Known Issues blocks after R86 ships. R87 task list built from those notes the same way R85/R86 were.
