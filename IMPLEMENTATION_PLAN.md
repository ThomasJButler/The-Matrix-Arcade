...
## Status: **R86 open** (2026-04-21 — Stream F mid-flight, F1 + F2 + F3 shipped). R86 = **Frogger + Neo Jump polish + 3 global regressions** (Batch 2 of the 3-batch per-game polish cadence started by R85). Loop cap: **20 iterations**. **Stream G all `[x]`**. **Stream F F1/F2/F3 `[x]`**. **R86.F3 `[x]`** — Kung Fu HUD "cut off" root-caused as geometric-not-stylistic: 24×24 icons centred at `baseY = HEIGHT-35 = 565` put their bottom edges at y=589, only 11px clear of the 600-tall canvas floor; the 7px label at y=551 also fused visually with the START safe-row backdrop and the player sprite. Fix relocates the HUD into the top-left gutter alongside Score/Distance/Combo (label y=75 at 10px, icons y=95), exploiting Frogger's 25° perspective taper which leaves `x<120` dark through rows 0-2 so the HUD never paints over a road lane. Label bumped from 7px → 10px for readability, alpha 0.6 → 0.8 to match the other HUD labels. +6 regression tests in a new `R86.F3 — Kung Fu HUD positioning` describe block (label placement + font size floor, icon count + shared baseline + left-gutter x-range, clearance-from-canvas-floor tripwire requiring ≥100px gap, label-above-icons ordering invariant, kungFuIcons wiring preserved). Sprite mock upgraded from `mockReturnValue` → `mockImplementation` so each `add.sprite` call returns a fresh mock (required because `createKungFuDisplay` pushes multiple sprites into an array and tests inspect them independently). Gates: tsc clean, lint 0 err (4 pre-existing warnings), **2797 tests pass + 2 todo** (+6 from R86.F2's 2791), `npm run build` clean 7.72s. Tom-tick awaited on next playtest — HUD should now read as a coherent top-left block, label visible, no clipping inside the iPod portal frame. **Next tasks**: R86.F4 (vehicle hit-box audit), R86.F5 (menu legend), parallel-safe Stream N (Neo Jump polish). Loop-sentinel rule preserved (no `\bCOMPLETE\b` on Status line mid-phase). **R85 awaiting sign-off** (Tom-tick only). R84.S7 `[DEFER]` + R84.CI bucket + R83.CTRLS umbrella also Tom-tick only. Full R85 archive at [`COMPLETED_WORK.md § R85`](COMPLETED_WORK.md#r85--invaders--metris-polish--2-globals-2026-04-20--2026-04-21).
