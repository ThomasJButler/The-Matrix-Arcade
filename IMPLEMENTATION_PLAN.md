...
## Status: **R86 open** (2026-04-22 — Stream F mid-flight, F1 + F2 + F3 + F4 + F5 shipped). R86 = **Frogger + Neo Jump polish + 3 global regressions** (Batch 2 of the 3-batch per-game polish cadence started by R85). Loop cap: **20 iterations**. **Stream G all `[x]`**. **Stream F F1/F2/F3/F4/F5 `[x]`**. **R86.F5 `[x]`** — Tom 2026-04-21 flagged "we need to have what the different game objects are in the game menu". `FroggerMenuScene` previously showed only the shared title/subtitle + a single "Arrow keys: Move | Goal: Cross all lanes without hitting Agents" line at y≈0.58, so a first-time player faced unfamiliar 64×64 agents/sentinels, three pickup-pill colours, and the kung-fu fist icon with no in-menu inventory. Fix adopts the Snake (R83) / Pong (R84) / Invaders (R85.I5) legend pattern: exported `HOW_TO_PLAY_Y_RATIO = 0.47`, `CONTROLS_Y_RATIO = 0.52`, `LEGEND_HEADING_Y_RATIO = 0.58`, `LEGEND_ICON_Y_RATIO = 0.64`, `LEGEND_LABEL_Y_RATIO = 0.675` (tightened from the initial 0.68 to preserve ≥10-px clearance from the shared START button at y=425 on 600-px canvas) + an exported `LEGEND_ITEMS` tuple of six `{ textureKey, label }` entries. Order is intentional: **AGENT → SENTINEL → KUNG FU → POINTS → POWER-UP → NEO** — danger first, your weapon middle, rewards last. Sprite keys (`enemy_agent`, `enemy_sentinel`, `kung_fu_icon`, `red_pill`, `blue_pill`, `neo_pickup`) all match textures registered in `FroggerBootScene` and a unit-test asserts that set so a future BootScene edit can't silently desync the legend. Icons are `setDisplaySize(22, 22)` so the 16-px vehicle frames, 64-px agent frames, and 32-px NEO orb all normalise to the same menu column. Column layout uses 60-px side padding with symmetric `(width - 120) / (columnCount - 1)` spacing — `xs[0] + xs[last] === canvas width`, enforced as a test invariant so a future asymmetric layout tweak fails gate. +18 regression tests in a new `MenuScene.test.ts` covering: ratio tuple in 0.40–0.70 band (×1), strictly increasing (×1), label y ≥10-px clearance from button top on 600-px canvas (×1), icon y + LEGEND_ICON_SIZE never overlaps button band (×1), all ratios < 0.70 anti-regression tripwire (×1), LEGEND_ITEMS length (×1), textureKey ⊂ BootScene key set (×1), label length ≤10 chars (×1), declared-order lock (×1), heading/controls/LEGEND strings emitted in order (×1), headings + rows on exported ratios (×1), exactly 6 icon sprites in declared order (×1), shared icon baseline (×1), symmetric X spacing (×1), setDisplaySize(22,22) per icon (×1), labels emitted in LEGEND_ITEMS order (×1), canvas-height scaling honoured (×1). Follows Invaders R85.I5 test harness pattern — manual prototype-chain rebinding + `super.create()` stub via temporary prototype override. Gates: tsc clean, lint 0 err (4 pre-existing warnings), **2821 tests pass + 2 todo** (+18 from R86.F4's 2803), `npm run build` clean 7.62s. Tom-tick awaited on next playtest — menu should show the full object roster before a round begins. **Next tasks**: R86.F6 (multi-level polish, contingent on F1 unlocking Level 2+), R86.F7 (unit-test coverage refresh), parallel-safe Stream N (Neo Jump polish). Loop-sentinel rule preserved (no `\bCOMPLETE\b` on Status line mid-phase). **R85 awaiting sign-off** (Tom-tick only). R84.S7 `[DEFER]` + R84.CI bucket + R83.CTRLS umbrella also Tom-tick only. Full R85 archive at [`COMPLETED_WORK.md § R85`](COMPLETED_WORK.md#r85--invaders--metris-polish--2-globals-2026-04-20--2026-04-21).
