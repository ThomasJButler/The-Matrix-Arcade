...
## Status: R82 open — Round 2 playtest refinements: R82.21–R82.28 [x] (portal top-clip + 16:9 full-fill + dashbar redesign + EXIT vs PAUSE split + Trophy icon on clickwheel bottom + Key panel right-of-iPod + bigger PLAY button on both clickwheel centre and dashbar centre + visual regression baselines regenerated). R82.29 (manual human playtest) still unchecked — Tom to action. R82.13 continuous-improvement resumed: dashbar mute pill upgraded from passive status `<div>` to interactive click-to-unmute `<button>` — `toggleMute` from `useSoundSystem` now plumbs App.tsx → GamePortal via new optional `onToggleMute` prop. When muted (and only when muted), the dashbar shows the same red-pulsing VolumeX+MUTED pill, but it's now tappable to recover audio without exiting the game. Kept conditional render (`{isMuted && …}`) so resting unmuted state is unchanged — portal visual baselines stay byte-identical (3/3 darwin portal specs pass clean). `.dashbar-mute` + `.dashbar-mute-btn` share the exact same default declarations (via comma-separated rule) so no default-state pixel drift; the button variant layers on `cursor: pointer`, hover-gated-behind-`(hover: hover)` brightness lift, `:focus-visible` outline, and `:active` 0.94 scale depression. Aria: `aria-label="Unmute audio"` + `aria-pressed="true"` + `title="Click to unmute"`. Disabled fallback when `onToggleMute` omitted so test fixtures and future consumers degrade gracefully. Muting during play still uses the global V shortcut + header Settings button (unchanged) — the dashbar deliberately doesn't grow a mute-when-unmuted affordance because that'd add a visible element in the most common state and force visual baseline regen for no real gain.
