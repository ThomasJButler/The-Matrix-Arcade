...
## Status: R82 open — R82.13 continuous-improvement: screen-reader keyboard-help discoverability. Visible keyboard hints under the clickwheel (`← → NAVIGATE • ↑ MENU...`) were ambient text — sighted users saw the shortcuts but SR users tabbing to the `[role="toolbar"]` wheel only heard the label ("Game navigation wheel — swipe left or right to navigate") with no enumeration of the actual keys. Added a sibling `<span id="ipod-wheel-instructions" className="sr-only">` with clean SR-friendly prose ("Arrow keys navigate: left and right switch games, up opens how to play..."; includes the 1-9 jump range parametrised on `games.length`, Home/End, Enter/Space, Escape). Wheel `motion.div` now carries `aria-describedby="ipod-wheel-instructions"` so focusing the wheel announces label + full keyboard help. Visual hint block wrapped with `aria-hidden="true"` to prevent SR users double-hearing the symbol-heavy visual version (arrow glyphs + bullets render unpredictably across NVDA/JAWS/VoiceOver). Both the sr-only span and the described-by target live inside the `!isPlaying` branch of the `AnimatePresence` ternary so they unmount cleanly with the wheel when the dashbar morphs in (no dangling ID references during play). Gates green (tsc clean, lint 0 errors, App.test.tsx 19/19 incl. visible-hints text assertion still works because sr-only content is in the a11y tree, not the text node, build 7.47s clean). Ralph does NOT write terminator — Tom's manual call post-review.
