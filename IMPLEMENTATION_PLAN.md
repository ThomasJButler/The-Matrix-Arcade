...
- [ ] **R86.R3 [P1]** **Menu HOW TO PLAY band overlaps RESONANCE (5th difficulty card)** — Tom's second screenshot shows the *"Q / W / O / P: Hit notes in time / Hold keys for hold notes / Goal: Hit falling fragments to the beat"* panel painting on top of what should be the `RESONANCE 110 BPM | 200s` card at the bottom of the 5-card difficulty list (faintly visible underneath the how-to-play text). Fix: audit `RhythmHackerMenuScene.ts` — either bump the HOW TO PLAY band's Y position BELOW the RESONANCE card's bottom edge with ≥10px clearance, OR shift the entire difficulty card list UP to make room. Prefer the former since 5 tracks is a design constant. If the 560-720px canvas is too short to fit both cleanly, shrink card vertical padding by ~10px each to claw back space. Ensure card hit-targets still work post-layout change (clickable area tracks text, not left-over from old position).
