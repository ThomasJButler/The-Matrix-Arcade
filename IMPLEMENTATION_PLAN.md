...
## Status: R82 open — R82.13 continuous-improvement: Framer Motion `useReducedMotion` gating shipped. Layout spring + dashbar/wheel AnimatePresence transitions now collapse to instant for users with `prefers-reduced-motion: reduce`, complementing existing CSS `@media` rules. Gates green (tsc clean, build 7.3s, App.test.tsx 19/19, portal visual 3/3). Ralph does NOT write terminator — Tom's manual call post-review.
...
- [ ] **R82.13 — [P3] ★ CONTINUOUS-IMPROVEMENT (never auto-checked, runs AFTER R82.14–R82.20)** — each remaining loop iteration picks ONE micro-improvement from the priority list below and ships it. This task is **intentionally never marked `[x]`** — Tom manually ticks it when he's happy with the final result. Ralph MUST NOT write the terminator phrase to Status; the `loop.sh` iteration cap is the stop signal. Tom's call (2026-04-18): *"fuck it we can make it the best version we possibly can"*.
  ...
  **R82.13 micro-improvement priority list** (pick one per iteration, commit as `R82.13: <improvement summary>`):
  ...
  - **Accessibility refinements** — ~~`prefers-reduced-motion` media query to disable wheel animations~~ ✅ JS side shipped 2026-04-16 (`useReducedMotion` hook in `GamePortal.tsx` gates Framer Motion `layout` spring + dashbar/wheel AnimatePresence transitions; CSS side already covered via `@media (prefers-reduced-motion: reduce)` block in `animations.css:638`); ~~screen-reader announcements on game change~~ ✅ shipped (commit 6e12092); focus-visible polish remaining
