...
## Status: R82 open — Round 2 playtest refinements: R82.21–R82.28 [x]. R82.29 (manual human playtest) still unchecked — Tom to action. R82.13 continuous-improvement resumed: dashbar keyboard hints exposed to screen readers — the in-play dashbar `motion.div` advertises `role="toolbar" aria-label="In-game controls"` but previously had no `aria-describedby`, so the freshly-shipped rove-nav keyboard shortcuts (ArrowLeft/Right rove, Home/End extremes, Enter/Space activate, Escape exit — commit abffe53) were invisible to assistive tech. A sighted mouse user sees hover hints via `.dashbar-btn` tooltips + `title` attrs; a screen-reader user landing on the toolbar would have heard only the name with no affordance cue. Now the dashbar motion.div carries `aria-describedby="ipod-dashbar-instructions"` and renders a co-located `<span id="ipod-dashbar-instructions" className="sr-only">` as its first DOM child: *"Left and right arrow keys move focus between controls. Home jumps to Exit, End jumps to the last control. Enter or Space activates the focused control. Escape exits the running game."* — word-for-word mirrors the clickwheel's long-established `ipod-wheel-instructions` span (line 692) so browse-mode and in-play now have identical SR onboarding scaffolding. **Why co-locate inside `motion.div`**: AnimatePresence `mode="wait"` keeps the exiting dashbar surface mounted through its ~260ms surface transition; stuffing the hint span outside (e.g. adjacent to the clickwheel's) would either orphan the ID when the clickwheel is mounted, or require a second `{isPlaying && ...}` block. Inside-motion.div means mount/unmount tracks the parent 1:1, with zero risk of a stale describedby target. **Why `aria-describedby` not label verbosity**: labels are names (NVDA/JAWS announce once when focus lands); descriptions fire after a short pause and only in verbose mode, exactly where keyboard hints belong. Stuffing hints into `aria-label="In-game controls. Use arrow keys..."` would make every focus event announce the whole paragraph — dashbar becomes unusable for power users. 2 new GamePortal tests lock in the attribute linkage, sr-only class, prose coverage (arrow/Home/End/Enter/Escape all verified in the text), and absence during browse mode (dashbar unmounted ⇒ hint span gone). **Gates all green**: lint 0 errors / 11 pre-existing warnings, tsc clean, 50/50 App + GamePortal + KeyPanel tests pass (22 GamePortal, 2 new), build clean (7.66s). No visual baselines affected — sr-only content has zero visual footprint. Previous change (dashbar keyboard rove nav during play): still deployed — this iteration makes that rove discoverable to non-sighted users.
